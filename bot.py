#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import sys,os,secrets,string,urllib.parse,base64,io,tempfile,json,hashlib,hmac,time as time_module,re,shutil,logging,traceback,random,asyncio,socket,subprocess,gc,sqlite3
...
import gzip,zipfile,platform,threading
...
def get_writable_path(base_path:Path,subdir:str)->Path:
    paths_to_try=[base_path/subdir,Path.home()/f".bot_{subdir}",Path(f"/tmp/bot_{subdir}"),Path(os.getenv('TEMP','/tmp'))/f"bot_{subdir}"]
...
logging.basicConfig(format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",level=logging.INFO,handlers=[RotatingFileHandler(LOG_PATH,maxBytes=10*1024*1024,backupCount=5,encoding='utf-8'),logging.StreamHandler()])
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    return await query.message.reply_text(text=plain,reply_markup=reply_markup,**kwargs)
                except:
                    raise final_e
...
class GlobalRateLimiter:
    def __init__(self):
        self.limits={'command':(5,10),'callback':(10,30),'message':(20,60),'api':(30,60)}
...
async def db_get_user_groups(user_id:int):
    async def _get(conn):
        try:
            cur=await conn.execute("SELECT DISTINCT bg.chat_id,bg.chat_name,bg.username,bg.banned FROM bot_groups bg WHERE bg.chat_id IN (SELECT chat_id FROM group_admins WHERE user_id=?) ORDER BY bg.chat_name",(user_id,))
            admin_groups=await cur.fetchall()
            cur=await conn.execute("SELECT DISTINCT bg.chat_id,bg.chat_name,bg.username,bg.banned FROM bot_groups bg INNER JOIN hidden_owner_groups hog ON bg.chat_id=hog.chat_id WHERE hog.owner_id=? ORDER BY bg.chat_name",(user_id,))
            owner_groups=await cur.fetchall()
            cur=await conn.execute("SELECT DISTINCT bg.chat_id,bg.chat_name,bg.username,bg.banned FROM bot_groups bg INNER JOIN hidden_admins ha ON bg.chat_id=ha.chat_id WHERE ha.admin_id=? ORDER BY bg.chat_name",(user_id,))
...
async def db_get_security_settings(chat_id:int,force_refresh:bool=False):
    default_settings={'links':False,'mentions':False,'warn':True,'slow_mode':False,'slow_mode_seconds':5,'welcome_enabled':False,'welcome_text':"مرحباً {user} في {chat} 🤍",'goodbye_enabled':False,'goodbye_text':"وداعاً {user} 👋",'delete_banned_words':False,'auto_penalty':'none','auto_mute_duration':60,'delete_videos':False,'delete_audio':False,'delete_animation':False,'delete_service':False,'delete_documents':False,'delete_stickers':False,'delete_penalty':'none','delete_penalty_duration':0}
        ...
        async def _get(conn):
            ...
            try:
                ...
                if 'mentions' not in columns:
                    ...
                    cur=await conn.execute("SELECT delete_links,mentions,warn_message,slow_mode,slow_mode_seconds,welcome_enabled,welcome_text,goodbye_enabled,goodbye_text,delete_banned_words,auto_penalty,auto_mute_duration,delete_videos,delete_audio,delete_animation,delete_service,delete_documents,delete_stickers,delete_penalty,delete_penalty_duration FROM group_security WHERE chat_id=?",(chat_id,))
                else:
                    cur=await conn.execute("SELECT delete_links,mentions,warn_message,slow_mode,slow_mode_seconds,welcome_enabled,welcome_text,goodbye_enabled,goodbye_text,delete_banned_words,auto_penalty,auto_mute_duration,delete_videos,delete_audio,delete_animation,delete_service,delete_documents,delete_stickers,delete_penalty,delete_penalty_duration FROM group_security WHERE chat_id=?",(chat_id,))
                row=await cur.fetchone()
                if row:
                    settings={'links':row['delete_links']==1,'mentions':row['mentions']==1,'warn':row['warn_message']==1,'slow_mode':row['slow_mode']==1,'slow_mode_seconds':row['slow_mode_seconds'] if row['slow_mode_seconds'] is not None else 5,'welcome_enabled':row['welcome_enabled']==1,'welcome_text':row['welcome_text'] if row['welcome_text'] else default_settings['welcome_text'],'goodbye_enabled':row['goodbye_enabled']==1,'goodbye_text':row['goodbye_text'] if row['goodbye_text'] else default_settings['goodbye_text'],'delete_banned_words':row['delete_banned_words']==1,'auto_penalty':row['auto_penalty'] if row['auto_penalty'] else 'none','auto_mute_duration':row['auto_mute_duration'] if row['auto_mute_duration'] is not None else 60,'delete_videos':row['delete_videos']==1,'delete_audio':row['delete_audio']==1,'delete_animation':row['delete_animation']==1,'delete_service':row['delete_service']==1,'delete_documents':row['delete_documents']==1,'delete_stickers':row['delete_stickers']==1,'delete_penalty':row['delete_penalty'] if 'delete_penalty' in row else 'none','delete_penalty_duration':row['delete_penalty_duration'] if 'delete_penalty_duration' in row else 0}
                    if CACHETOOLS_AVAILABLE:
                    ...
                    return settings
                await ensure_security_columns(conn)
                await conn.execute("INSERT INTO group_security (chat_id,delete_links,mentions,warn_message,slow_mode,slow_mode_seconds,welcome_enabled,welcome_text,goodbye_enabled,goodbye_text,delete_banned_words,auto_penalty,auto_mute_duration,delete_videos,delete_audio,delete_animation,delete_service,delete_documents,delete_stickers,delete_penalty,delete_penalty_duration) VALUES (?,0,0,1,0,5,0,?,0,?,0,'none',60,0,0,0,0,0,0,'none',0)",(chat_id,default_settings['welcome_text'],default_settings['goodbye_text']))
                await conn.commit()
                if CACHETOOLS_AVAILABLE:
    ...
    try:
        async def _set(conn):
            ...
            if not exists:
                ...
                await conn.execute("INSERT INTO group_security (chat_id,delete_links,mentions,warn_message,slow_mode,slow_mode_seconds,welcome_enabled,welcome_text,goodbye_enabled,goodbye_text,delete_banned_words,auto_penalty,auto_mute_duration,delete_videos,delete_audio,delete_animation,delete_service,delete_documents,delete_stickers,delete_penalty,delete_penalty_duration) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(chat_id,1 if final_settings.get('links',False) else 0,1 if final_settings.get('mentions',False) else 0,1 if final_settings.get('warn',True) else 0,1 if final_settings.get('slow_mode',False) else 0,final_settings.get('slow_mode_seconds',5),1 if final_settings.get('welcome_enabled',False) else 0,final_settings.get('welcome_text',default_settings['welcome_text']),1 if final_settings.get('goodbye_enabled',False) else 0,final_settings.get('goodbye_text',default_settings['goodbye_text']),1 if final_settings.get('delete_banned_words',False) else 0,final_settings.get('auto_penalty','none'),final_settings.get('auto_mute_duration',60),1 if final_settings.get('delete_videos',False) else 0,1 if final_settings.get('delete_audio',False) else 0,1 if final_settings.get('delete_animation',False) else 0,1 if final_settings.get('delete_service',False) else 0,1 if final_settings.get('delete_documents',False) else 0,1 if final_settings.get('delete_stickers',False) else 0,final_settings.get('delete_penalty','none'),final_settings.get('delete_penalty_duration',0)))
                await conn.commit()
                if chat_id in _security_cache:
...
async def db_get_hidden_admins_for_user(user_id:int):
    async def _get(conn):
        cur=await conn.execute("SELECT chat_id,added_by,added_at FROM hidden_admins WHERE admin_id=?",(user_id,))
        return await cur.fetchall()
    return await execute_db(_get)
# ===================== الجدولة =====================
...
async def db_auto_reward_referral(referrer_id:int,referred_id:int)->int:
    async def _reward(conn):
        ...
        await conn.execute("INSERT INTO referral_rewards (user_id,referral_count,total_reward_days,claimed_reward_days) VALUES (?,0,?,0) ON CONFLICT(user_id) DO UPDATE SET referral_count=referral_count+1,total_reward_days=total_reward_days+?",(referrer_id,reward_days,reward_days))
...
async def db_create_contest(creator_id:int,title:str,description:str,prize:str,end_date:datetime,contest_type:str='raffle')->int:
    try:
        async def _create(conn):
            ...
            cur=await conn.execute("INSERT INTO contests (creator_id,title,description,prize,end_date,status,created_at,contest_type) VALUES (?,?,?,?,?,?,?,?)",(creator_id,title,description,prize,end_date_str,'active',created_at_str,contest_type))
            await conn.commit()
...
async def db_get_contest(contest_id:int)->dict|None:
    async def _get(conn):
        ...
        row=await cur.fetchone()
        if row:
            return {'id':row['id'],'title':row['title'],'description':row['description'],'prize':row['prize'],'end_date':row['end_date'],'status':row['status'],'winner_id':row['winner_id'],'creator_id':row['creator_id'],'created_at':row['created_at'],'contest_type':row['contest_type'] if 'contest_type' in row else 'raffle'}
        return None
    return await execute_db(_get)
...
def get_banned_words_admin_keyboard():
    return InlineKeyboardMarkup([[InlineKeyboardButton("➕ إضافة كلمة عامة",callback_data=CallbackData.ADMIN_ADD_BANNED_WORD),InlineKeyboardButton("📋 عرض الكلمات",callback_data=CallbackData.ADMIN_LIST_BANNED_WORDS)],[InlineKeyboardButton("🗑️ حذف كلمة",callback_data=CallbackData.ADMIN_REMOVE_BANNED_WORD),InlineKeyboardButton("🔙 رجوع",callback_data=CallbackData.ADMIN_BANNED_WORDS)]])
...
# ===================================================================
# دوال الأمان والحماية (security_*)
# ===================================================================
...
async def security_bulk_toggle(update:Update,context:ContextTypes.DEFAULT_TYPE,enabled:bool):
    ...
    if not await is_authorized_in_group(context.bot,chat_id,user_id):
        ...
        return
    if enabled:
        kb=InlineKeyboardMarkup([[InlineKeyboardButton("✅ نعم، تفعيل الكل",callback_data=f"confirm_enable_all:{chat_id}")],[InlineKeyboardButton("❌ إلغاء",callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}")]])
...
async def security_delete_penalty_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_authorized_in_group(context.bot,chat_id,user_id):
        await query.answer(get_text(user_id,'admin_only'),show_alert=True)
        return
    keyboard=InlineKeyboardMarkup([[InlineKeyboardButton("🚫 لا شيء",callback_data=f"set_delete_penalty:none:{chat_id}"),InlineKeyboardButton("👢 طرد",callback_data=f"set_delete_penalty:kick:{chat_id}")],[InlineKeyboardButton("🛑 حظر",callback_data=f"set_delete_penalty:ban:{chat_id}"),InlineKeyboardButton("🔇 كتم",callback_data=f"set_delete_penalty:mute:{chat_id}")],[InlineKeyboardButton("🔙 رجوع إلى الأمان",callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}")]])
    await query.edit_message_text("⚖️ **اختر عقوبة حذف المحتوى**\n\nسيتم تطبيق هذه العقوبة عند حذف أي محتوى مخالف.",reply_markup=keyboard,parse_mode="Markdown")
...
async def trial_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if await db_has_used_trial(uid):
        if query:
            ...
            await safe_send_markdown(context.bot,uid,get_text(uid,'trial_used'))
        return
    if await db_has_active_subscription(uid):
        if query:
            await query.edit_message_text(get_text(uid,'already_subscribed'))
        else:
            await safe_send_markdown(context.bot,uid,get_text(uid,'already_subscribed'))
        return
    await db_activate_trial(uid)
    if query:
        await query.edit_message_text(get_text(uid,'trial'))
    else:
        await safe_send_markdown(context.bot,uid,get_text(uid,'trial'))
    await main_menu_callback(update,context)
...
async def translation_off_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,get_text(uid,'translation_disabled'),reply_markup=kb)
async def translation_set_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,get_text(uid,'translation_enabled').format(lang_name),reply_markup=kb)
# ===================================================================
...
async def admin_users_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not users:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا يوجد مستخدمون مسجلون.",reply_markup=kb)
        return
...
async def admin_banned_users_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not banned_users:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا يوجد مستخدمون محظورون.",reply_markup=kb)
        return
...
async def admin_unban_all_users_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"✅ تم إلغاء حظر جميع المستخدمين.",reply_markup=kb)
async def admin_all_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد قنوات مسجلة.",reply_markup=kb)
        return
...
async def admin_banned_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد قنوات محظورة.",reply_markup=kb)
        return
...
async def admin_activate_all_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"✅ تم إلغاء حظر جميع قنوات المستخدمين.",reply_markup=kb)
async def admin_groups_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not groups:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد مجموعات مسجلة.",reply_markup=kb)
        return
...
async def admin_banned_groups_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not groups:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد مجموعات محظورة.",reply_markup=kb)
        return
...
async def admin_unban_all_groups_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"✅ تم إلغاء حظر جميع المجموعات.",reply_markup=kb)
async def admin_bot_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد قنوات أضيف إليها البوت.",reply_markup=kb)
        return
    text="📢 **قنوات البوت**\n━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def admin_banned_bot_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد قنوات بوت محظورة.",reply_markup=kb)
        return
...
async def admin_unban_all_bot_channels_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"✅ تم إلغاء حظر جميع قنوات البوت.",reply_markup=kb)
...
async def admin_add_admin_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"👑 أرسل معرف المستخدم (user_id) لإضافته كمشرف:")
async def admin_remove_admin_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not admins:
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا يوجد مشرفون لإزالتهم.")
        return
...
async def admin_backup_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"✅ تم إنشاء نسخة احتياطية مشفرة جديدة.",reply_markup=kb)
    except Exception as e:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,f"❌ فشل إنشاء النسخة (الرمز: `{error_id}`)",reply_markup=kb)
async def admin_restore_backup_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not backups:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد نسخ احتياطية.",reply_markup=kb)
        return
    kb=[]
    for b in backups[:10]:
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"📂 **اختر النسخة الاحتياطية للاستعادة:**",reply_markup=InlineKeyboardMarkup(kb))
async def admin_restore_backup_select_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"✅ تم استعادة النسخة الاحتياطية المشفرة.",reply_markup=kb)
    except Exception as e:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,f"❌ فشل الاستعادة (الرمز: `{error_id}`)",reply_markup=kb)
...
async def admin_toggle_auto_backup_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,f"✅ تم تغيير إعداد النسخ التلقائي إلى: {status}",reply_markup=kb)
...
async def admin_force_subscribe_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,f"✅ تم {status_text} الاشتراك الإجباري.",reply_markup=kb)
...
async def admin_confirm_broadcast_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not broadcast_text:
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"❌ لا يوجد نص للإرسال")
        return
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern,broadcast_text,re.IGNORECASE):
            if query:
                ...
                await safe_send_markdown(context.bot,uid,"❌ النص يحتوي على كود ضار! تم منع الإرسال.")
            return
    if len(broadcast_text)>4000:
        if query:
            await query.edit_message_text("❌ النص طويل جداً (الحد الأقصى 4000 حرف)")
        else:
            await safe_send_markdown(context.bot,uid,"❌ النص طويل جداً (الحد الأقصى 4000 حرف)")
        return
    if query:
        await query.edit_message_text("📨 جاري الإرسال... يرجى الانتظار")
    else:
        await safe_send_markdown(context.bot,uid,"📨 جاري الإرسال... يرجى الانتظار")
    ...
    if not users:
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا يوجد مستخدمين نشطين لإرسال الرسالة لهم.")
        return
...
async def admin_support_tickets_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not tickets:
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد تذاكر دعم مسجلة")
        return
    text="📋 **تذاكر الدعم**\n━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def admin_delete_all_tickets_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,"⚠️ **تأكيد حذف جميع التذاكر**\n\nهل أنت متأكد من حذف جميع تذاكر الدعم؟",reply_markup=confirm_kb)
async def admin_confirm_delete_tickets_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,f"✅ تم حذف {count} تذكرة بنجاح.",reply_markup=kb)
async def admin_manage_sendcode_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    keyboard=InlineKeyboardMarkup([[InlineKeyboardButton("➕ تعيين مستخدم جديد",callback_data=CallbackData.ADMIN_SET_SENDCODE_USER)],[InlineKeyboardButton("🔙 رجوع",callback_data=CallbackData.ADMIN_PANEL)]])
    if query:
...
async def admin_replies_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,msg,reply_markup=get_replies_keyboard())
...
async def admin_list_replies_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not replies:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد ردود مسجلة.",reply_markup=kb)
        return
...
async def admin_banned_words_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,msg,reply_markup=get_banned_words_admin_keyboard())
...
async def admin_list_banned_words_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not words:
        ...
        if query:
            ...
            await safe_send_markdown(context.bot,uid,"📭 لا توجد كلمات محظورة عامة.",reply_markup=kb)
        return
...
async def admin_del_banned_word_callback(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await safe_send_markdown(context.bot,uid,f"✅ تم حذف {word}")
...
# ===================================================================
# دوال الردود التلقائية والإعدادات المتقدمة
# ===================================================================
...
async def is_user_subscribed(bot,user_id,channel):
    ...
    try:
        ...
        return False
# ===================================================================
# دوال لوحة التحكم (قفل/فتح وإجراءات متقدمة)
# ===================================================================
...
# ===================================================================
# دوال معالجات الأحداث (ChatMember, NewChatMembers, LeftChatMember)
# ===================================================================
async def send_addition_report_to_all_admins(bot,chat,adder,chat_type_name):
    try:
        ...
        for admin in admins:
            ...
            if user.id==adder.id:
                try:
                    await bot.send_message(chat_id=user.id,text=(f"✅ **تم إضافة البوت إلى {chat_type_name}**\n\n"
                                                                    f"📌 الاسم: {chat.title}\n"
                                                                    f"🆔 المعرف: {chat.id}\n"
                                                                    ...
                                                                    f"📌 **ملاحظة:** إذا لم تظهر لك المجموعة، استخدم /syncgroup في المجموعة"),parse_mode="MarkdownV2")
                ...
                try:
                    await bot.send_message(chat_id=user.id,text=(f"📢 **تم إضافة البوت إلى {chat_type_name}**\n\n"
                                                                    ...
                                                                    f"🔹 **ملاحظة:** إذا كنت تريد إدارة البوت، تأكد من أنك مشرف في المجموعة."),parse_mode="MarkdownV2")
    ...
    if new_status in [ChatMember.MEMBER,ChatMember.ADMINISTRATOR,ChatMember.OWNER]:
        ...
        if is_new:
            ...
            if chat.type=='channel':
                ...
                chat_type_name="قناة"
                try:
                    await context.bot.send_message(chat_id=adder.id,text=(f"✅ **تم إضافة البوت إلى {chat_type_name}**\n\n"
                                                                            f"📌 الاسم: {chat.title}\n"
                                                                            f"🆔 المعرف: {chat.id}\n"
                                                                            f"📢 يمكنك الآن استخدام البوت لإدارة القناة."),parse_mode="MarkdownV2")
                except:
                    pass
...
async def successful_payment_callback_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_markdown(context.bot,uid,f"✅ **تم تفعيل اشتراكك لمدة {days} يوماً!**\nشكراً لدعمك ❤️")
# ===================================================================
# دوال حذف رسائل الخدمة وتصفية الرسائل ومعالج الرسائل الرئيسي
# ===================================================================
async def delete_service_messages(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    service_flags=[message.new_chat_members,message.left_chat_member,message.new_chat_photo,message.delete_chat_photo,message.group_chat_created,message.supergroup_chat_created,message.channel_chat_created,message.migrate_to_chat_id,message.migrate_from_chat_id,message.pinned_message,message.successful_payment,message.invoice,message.connected_website,message.boost_added]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "message can't be deleted" in error_msg:
                ...
                try:
                    await context.bot.send_message(chat_id=chat_id,text="⚠️ **تنبيه:** البوت يحتاج صلاحية 'حذف الرسائل' ليعمل بشكل صحيح.\nيرجى منح البوت الصلاحيات المطلوبة.",parse_mode="MarkdownV2")
                except:
                    pass
                return False
...
async def message_handler_main(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if state==UserState.WAITING_CHANNEL_ID:
        ...
        context.user_data.pop('state',None)
        keyboard=InlineKeyboardMarkup([[InlineKeyboardButton("✅ تأكيد الإرسال",callback_data=CallbackData.ADMIN_CONFIRM_BROADCAST),InlineKeyboardButton("❌ إلغاء",callback_data=CallbackData.ADMIN_PANEL)]])
...
async def handle_sendcode_confirmation_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not expected_password:
        await safe_send_markdown(context.bot,user_id,"❌ لم يتم طلب إرسال كود")
        context.user_data.pop('state',None)
        return
    SENDCODE_TIMEOUT=600
    if time_module.time()-timestamp>SENDCODE_TIMEOUT:
        await safe_send_markdown(context.bot,user_id,f"❌ انتهت صلاحية كلمة المرور (المهلة {SENDCODE_TIMEOUT//60} دقائق).\nأعد استخدام الأمر /sendcode.")
        context.user_data.pop('sendcode_temp_password',None)
    ...
    if update.message.text.strip()==expected_password:
        try:
            with open(__file__,'r',encoding='utf-8') as f:
                content=f.read()
            content=re.sub(r'TOKEN\s*=\s*["\'][^"\']+["\']','TOKEN = "[REMOVED]"',content)
            content=re.sub(r'ENCRYPTION_KEY\s*=\s*[^;]+','ENCRYPTION_KEY = "[REMOVED]"',content)
            content=re.sub(r'BACKUP_ENCRYPTION_KEY\s*=\s*[^;]+','BACKUP_ENCRYPTION_KEY = "[REMOVED]"',content)
            content=re.sub(r'cipher_suite\s*=\s*[^;]+','cipher_suite = "[REMOVED]"',content)
            content=re.sub(r'BACKUP_CIPHER\s*=\s*[^;]+','BACKUP_CIPHER = "[REMOVED]"',content)
            watermark=f"""# ============================================================
...
"""
            ...
            await safe_send_markdown(context.bot,user_id,f"❌ فشل إرسال الكود: {str(e)[:100]}")
...
async def index_handler(request):
    html_content="""<html><head><title>ريلاكس مانيجر</title></head><body style="font-family:Arial;text-align:center;padding:50px;direction:rtl;"><h1>🌿 ريلاكس مانيجر</h1><p>✅ البوت يعمل بكفاءة</p><p>📊 <a href="/health">التحقق من الصحة</a></p><p>🤖 <a href="https://t.me/Reelaaaxbot">البوت على تيليجرام</a></p><p style="color:#666;font-size:12px;">الإصدار 20.0.19</p></body></html>"""
...
# ===================================================================
# دوال مساعدة إضافية (main_menu, back, handle_text, إلخ)
# ===================================================================
...
async def handle_text_callbacks(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if data=="rank":
        ...
        text=f"📊 **رتبتك**\n━━━━━━━━━━━━━━━━━━━━━━\n📌 المستوى: {rank_data['level']}\n⭐ النقاط: {rank_data['points']}\n\nالمستوى التالي: {rank_data['level']+1} يحتاج {LEVEL_REQUIREMENTS.get(rank_data['level']+1,'غير محدود')} نقطة"
        await safe_edit_markdown(query,text)
    elif data=="top":
        ...
        msg="📝 **جدولة منشور**\n\nأرسل التاريخ والوقت والنص بهذه الصيغة:\n`YYYY-MM-DD HH:MM نص المنشور`\n\nمثال: `2025-12-25 14:30 عيد ميلاد سعيد`\n\n⏰ الوقت بتوقيت مكة المكرمة"
        await safe_edit_markdown(query,msg)
async def lang_callback_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if lang in SUPPORTED_LANGUAGES:
        ...
        await query.edit_message_text("⚠️ لغة غير مدعومة")
...
async def self_ping_loop():
    ...
    while True:
        try:
            await asyncio.sleep(300)
            async with aiohttp.ClientSession() as session:
                url=f"http://localhost:{WEB_PORT_USED}/health"
...
async def init_db_improved():
    async with aiosqlite.connect(str(DB_PATH),timeout=DB_TIMEOUT) as conn:
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY,auto_publish INTEGER DEFAULT 1,banned INTEGER DEFAULT 0,trial_used INTEGER DEFAULT 0,subscription_end TEXT DEFAULT NULL,referral_code TEXT DEFAULT NULL,active_channel INTEGER DEFAULT NULL,auto_reply_enabled INTEGER DEFAULT 1,auto_recycle INTEGER DEFAULT 1,last_daily_reward TEXT DEFAULT NULL,last_weekly_reward TEXT DEFAULT NULL,achievements TEXT DEFAULT '[]')")
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS group_security (chat_id INTEGER PRIMARY KEY,delete_links INTEGER DEFAULT 0,mentions INTEGER DEFAULT 0,warn_message INTEGER DEFAULT 1,slow_mode INTEGER DEFAULT 0,slow_mode_seconds INTEGER DEFAULT 5,welcome_enabled INTEGER DEFAULT 0,welcome_text TEXT DEFAULT 'مرحباً {user} في {chat} 🤍',goodbye_enabled INTEGER DEFAULT 0,goodbye_text TEXT DEFAULT 'وداعاً {user} 👋',delete_banned_words INTEGER DEFAULT 0,auto_penalty TEXT DEFAULT 'none',auto_mute_duration INTEGER DEFAULT 60,delete_videos INTEGER DEFAULT 0,delete_audio INTEGER DEFAULT 0,delete_animation INTEGER DEFAULT 0,delete_service INTEGER DEFAULT 0,delete_documents INTEGER DEFAULT 0,delete_stickers INTEGER DEFAULT 0,delete_penalty TEXT DEFAULT 'none',delete_penalty_duration INTEGER DEFAULT 0)")
        await ensure_security_columns(conn)
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS group_admins (chat_id INTEGER NOT NULL,user_id INTEGER NOT NULL,PRIMARY KEY(chat_id,user_id))")
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS group_rules (chat_id INTEGER PRIMARY KEY,rules_text TEXT,set_by INTEGER,set_at TIMESTAMP)")
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS blocked_users (user_id INTEGER PRIMARY KEY,reason TEXT,blocked_by INTEGER,blocked_at TIMESTAMP,expires_at TIMESTAMP,severity TEXT DEFAULT 'ban')")
...
async def main():
    ...
    commands=[BotCommand("start","بدء البوت"),BotCommand("trial","تجربة مجانية"),BotCommand("subscribe","الاشتراك"),BotCommand("syncgroup","تفعيل المجموعة"),BotCommand("security","إعدادات الأمان"),BotCommand("register_hidden_owner","تسجيل مالك مخفي"),BotCommand("add_hidden_admin","إضافة مشرف مخفي"),BotCommand("remove_hidden_admin","إزالة مشرف مخفي"),BotCommand("list_hidden_admins","عرض المشرفين المخفيين"),BotCommand("rank","رتبتك"),BotCommand("top","أفضل 10"),BotCommand("stats","إحصائيات القناة"),BotCommand("lock","قفل المجموعة"),BotCommand("unlock","فتح المجموعة"),BotCommand("schedule","جدولة منشور"),BotCommand("panel","لوحة التحكم"),BotCommand("language","تغيير اللغة"),BotCommand("support","مركز الدعم"),BotCommand("support_reply","الرد على تذكرة"),BotCommand("help","المساعدة"),BotCommand("developer","المطور"),BotCommand("updates","آخر التحديثات"),BotCommand("sendcode","إرسال كود البوت"),BotCommand("set_log_channel","تعيين قناة التقارير"),BotCommand("ban","حظر مستخدم"),BotCommand("mute","كتم مستخدم"),BotCommand("warn","تحذير مستخدم"),BotCommand("kick","طرد مستخدم"),BotCommand("restrict","تقييد مستخدم"),BotCommand("pin","تثبيت رسالة"),BotCommand("unban","إلغاء حظر مستخدم"),BotCommand("contests","المسابقات"),BotCommand("create_contest","إنشاء مسابقة"),BotCommand("declare_winner","إعلان فائز"),BotCommand("set_rules","تعيين قوانين المجموعة"),BotCommand("rules","عرض قوانين المجموعة")]
    ...
    task_manager.create_task(refresh_group_admins_and_hidden_owners_loop(application.bot))
