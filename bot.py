...
class MessageHandlers:
    ...
    async def handle_private(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # الحالة غير المعروفة - فقط في الخاص
        if update.effective_chat and update.effective_chat.type == 'private':
            await CommandHandlers.start(update, context)

    @staticmethod
    async def handle_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """معالجة الرسائل في المجموعات (الردود التلقائية فقط، مع فحص سريع)"""
        if not update.message or not update.effective_user:
            return
        ...
        # فحص سريع: هل الردود التلقائية مفعلة؟
        ars = await AutoReplyRepository.get_settings(chat_id)
        if not ars.get('enabled'):
            return

        # تحقق من الصلاحية (إذا كانت للمشرفين فقط)
        if ars.get('only_admins'):
            if not await is_authorized_in_group(context.bot, chat_id, user_id):
                return

        # تجاهل البوتات إذا كان الإعداد مطلوباً
        ...
        # تحليل المشاعر (اختياري، مع تجاهل الأخطاء)
        if len(text) > 3:
            try:
