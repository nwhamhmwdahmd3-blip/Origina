...
from telegram.ext import (
    ...
    filters, ContextTypes, PreCheckoutQueryHandler, ChatMemberHandler,
...
async def db_get_subscription_days_left(user_id: int) -> int:
    async def _g(conn):
        ...
        if row and row[0]:
            try:
                return max(0, (datetime.fromisoformat(row[0]) - utc_now()).days)
            except:
                pass
        return 0
...
async def db_set_auto(user_id: int, enabled: bool):
    await execute_db(lambda c: c.execute("UPDATE users SET auto_publish=? WHERE user_id=?",
                                         (1 if enabled else 0, user_id)) or c.commit())
...
async def db_set_auto_recycle(user_id: int, enabled: bool):
    await execute_db(lambda c: c.execute("UPDATE users SET auto_recycle=? WHERE user_id=?",
                                         (1 if enabled else 0, user_id)) or c.commit())
...
async def db_update_user_cache(user_id: int, username: str, first_name: str):
    await execute_db(lambda c: c.execute(
...
async def db_add_channel(user_id: int, channel_id: str, channel_name: str) -> int:
    async def _add(conn):
        cur = await conn.execute("SELECT id FROM user_channels WHERE user_id=? AND channel_id=?",
                                 (user_id, channel_id))
        if await cur.fetchone():
            return None
...
async def db_get_channels(user_id: int):
    return await execute_db(lambda c: c.execute(
        ...
        (user_id,)) or c.fetchall())

async def db_get_channel_info(channel_db_id: int):
    return await execute_db(lambda c: c.execute(
        "SELECT channel_id, channel_name FROM user_channels WHERE id=?", (channel_db_id,)) or c.fetchone())
...
async def db_get_active_channel(user_id: int):
    async def _g(conn):
        ...
        if row and row[0]:
            ...
            if r2 and r2[0] == 0:
                return row[0]
        cur = await conn.execute("SELECT id FROM user_channels WHERE user_id=? AND banned=0 ORDER BY id LIMIT 1",
                                 (user_id,))
        row = await cur.fetchone()
        return row[0] if row else None
    return await execute_db(_g)
...
async def db_set_active_channel(user_id: int, channel_db_id: int):
    await execute_db(lambda c: c.execute("UPDATE users SET active_channel=? WHERE user_id=?",
                                         (channel_db_id, user_id)) or c.commit())
...
async def db_get_next_post(channel_db_id: int):
    async def _g(conn):
        cur = await conn.execute(
            "SELECT id, text, media_type, media_file_id FROM posts WHERE channel_db_id=? AND published=0 "
            "AND (fail_count IS NULL OR fail_count < 3) ORDER BY id LIMIT 1",
            (channel_db_id,))
        row = await cur.fetchone()
        if row:
...
async def db_mark_published(post_id: int):
    await execute_db(lambda c: c.execute("UPDATE posts SET published=1, published_at=? WHERE id=?",
                                         (utc_now_iso(), post_id)) or c.commit())
...
async def db_get_user_posts_for_channel(channel_db_id: int, limit=15):
    return await execute_db(lambda c: c.execute(
        ...
        (channel_db_id, limit)) or c.fetchall())

async def db_delete_single_post(post_id: int, user_id: int, channel_db_id: int) -> bool:
    async def _d(conn):
        cur = await conn.execute("SELECT 1 FROM user_channels WHERE id=? AND user_id=? AND banned=0",
...
async def db_increment_fail_count(post_id: int):
    await execute_db(lambda c: c.execute("UPDATE posts SET fail_count=COALESCE(fail_count,0)+1 WHERE id=?",
                                         (post_id,)) or c.commit())
    ...
    async def _reg(conn):
        ...
        if existing:
            await conn.execute("UPDATE bot_groups SET chat_name=?, username=?, added_by=? WHERE chat_id=?",
                               (chat_name[:255], username[:100] if username else None, added_by, chat_id))
            await conn.commit()
...
async def db_get_user_groups(user_id: int):
    async def _g(conn):
        result = []
        seen = set()
        for table, col in [("hidden_owner_groups", "owner_id"),
                           ("hidden_admins", "admin_id"),
                           ("group_admins", "user_id")]:
            cur = await conn.execute(
                f"SELECT DISTINCT bg.chat_id, bg.chat_name, bg.username, bg.banned "
                f"FROM bot_groups bg INNER JOIN {table} h ON bg.chat_id=h.chat_id WHERE h.{col}=?",
                (user_id,))
...
async def db_sync_group_admins(chat_id: int, bot, owner_id: int = None) -> int:
    try:
        ...
        async def _upd(conn):
            ...
            await conn.executemany("INSERT OR IGNORE INTO group_admins (chat_id, user_id) VALUES (?,?)",
...
async def check_bot_admin_permissions_group(bot, chat_id: int) -> dict:
    try:
        ...
        if not perms['can_delete'] or not perms['can_ban']:
            return {'can_act': False, 'reason': 'ينقص البوت صلاحيات'}
        return {'can_act': True, 'reason': '', 'permissions': perms}
    except Exception:
        return {'can_act': False, 'reason': 'خطأ'}
...
async def is_currently_admin_in_group(bot, chat_id: int, user_id: int) -> bool:
    try:
        if user_id == ANONYMOUS_ADMIN_ID:
            return len(await bot.get_chat_administrators(chat_id)) > 0
        ...
        if member.status in ['administrator', 'creator']:
            ...
            async def _chk(conn):
                for table, col in [("hidden_owner_groups", "owner_id"),
                                   ("hidden_admins", "admin_id"),
                                   ("group_admins", "user_id")]:
                    cur = await conn.execute(f"SELECT 1 FROM {table} WHERE chat_id=? AND {col}=?",
                                             (chat_id, user_id))
                    if await cur.fetchone():
                        return True
                return False
        ...
        async def _chk2(conn):
            for table, col in [("hidden_owner_groups", "owner_id"),
                ...
                if await cur.fetchone():
                    return True
            return False
        authorized = await execute_db(_chk2)
...
async def db_get_security_settings(chat_id: int, force_refresh: bool = False) -> dict:
    ...
    if not force_refresh and CACHETOOLS_AVAILABLE and chat_id in _security_cache:
        ...
        if time_module.time() - ct < _AUTH_CACHE_TTL:
            return val.copy()
    async def _g(conn):
...
async def db_check_slow_mode(chat_id: int, user_id: int) -> bool:
    ...
    async def _chk(conn):
        cur = await conn.execute("SELECT message_time FROM user_messages WHERE chat_id=? AND user_id=?",
                                 (chat_id, user_id))
        row = await cur.fetchone()
        now = utc_now()
        if row:
...
async def db_remove_banned_word(word: str, chat_id: int) -> bool:
    async def _r(conn):
        await conn.execute("DELETE FROM banned_words WHERE word=? AND chat_id=?",
                           (word.strip().lower(), chat_id))
        await conn.commit()
        if chat_id == -1:
            await rebuild_banned_patterns()
        return True
...
async def apply_penalty_with_duration(bot, chat_id: int, user_id: int, penalty: str,
                                      ...
                                      moderator_id: int = None) -> Tuple[bool, str]:
    if user_id == PRIMARY_OWNER_ID:
    ...
    try:
        if penalty == 'ban':
            ...
            # مبسطة: إنشاء سجل تحذير
            async def _warn(conn):
                await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, moderator_id, reason, created_at) VALUES (?,?,'warn',?,?,?)",
                                   (chat_id, user_id, moderator_id, reason, utc_now_iso()))
                await conn.commit()
            await execute_db(_warn)
        ...
        async def _log(conn):
            await conn.execute(
                ...
                (chat_id, user_id, penalty, duration_minutes if penalty == 'mute' else 0,
                 moderator_id, reason[:200] if reason else "", utc_now_iso()))
            await conn.commit()
...
async def delete_and_penalize(update: Update, context: ContextTypes.DEFAULT_TYPE, warning_message: str):
    ...
    if penalty != 'none':
        await apply_penalty_with_duration(context.bot, chat_id, user_id, penalty,
...
async def get_moderation_log(chat_id: int, limit: int = 20) -> str:
    async def _g(conn):
        conn.row_factory = aiosqlite.Row
        cur = await conn.execute(
            "SELECT user_id, action, duration_minutes, reason, created_at, moderator_id "
            "FROM moderation_log WHERE chat_id=? ORDER BY created_at DESC LIMIT ?",
            (chat_id, limit))
        return await cur.fetchall()
    logs = await execute_db(_g)
    if not logs:
    ...
    for log in logs:
        ...
        action_names = {'ban': '🚫 حظر', 'mute': '🔇 كتم', 'warn': '⚠️ تحذير',
                        'kick': '👢 طرد', 'restrict': '🔒 تقييد', 'unban': '🔓 إلغاء حظر'}
        ...
        mod_text = f"\n   👤 بواسطة: `{mod_id}`" if mod_id else ""
        text += f"• `{uid}` → {action_text}{duration_text}{reason_text}{mod_text}\n   🕐 {time_str}\n\n"
    return text

# ===================================================================
# 16. دوال الإحالات والمسابقات والتذاكر
# ===================================================================
...
async def db_auto_reward_referral(referrer_id: int, referred_id: int) -> int:
    async def _r(conn):
        await conn.execute(
            ...
            "referral_count=referral_count+1, total_reward_days=total_reward_days+3",
            (referrer_id,))
        await conn.commit()
        return 3
    return await execute_db(_r)
...
async def db_get_referral_stats(user_id: int) -> dict:
    async def _g(conn):
        ...
        row = await cur.fetchone()
        if row:
            return {'total_referrals': total, 'referral_count': row[0],
                    'total_reward_days': row[1], 'claimed_reward_days': row[2],
                    'available_days': row[1] - row[2]}
        return {'total_referrals': total, 'referral_count': 0, 'total_reward_days': 0,
                'claimed_reward_days': 0, 'available_days': 0}
    return await execute_db(_g)
...
async def db_get_contest(contest_id: int):
    async def _g(conn):
        cur = await conn.execute(
            "SELECT id, title, description, prize, end_date, status, winner_id FROM contests WHERE id=?",
            (contest_id,))
        row = await cur.fetchone()
        if row:
            return {'id': row[0], 'title': row[1], 'description': row[2], 'prize': row[3],
                    'end_date': row[4], 'status': row[5], 'winner_id': row[6]}
        return None
    return await execute_db(_g)
...
async def db_get_user_participation(user_id: int, contest_id: int) -> bool:
    async def _chk(conn):
        cur = await conn.execute("SELECT 1 FROM contest_participants WHERE contest_id=? AND user_id=?",
                                 (contest_id, user_id))
        return await cur.fetchone() is not None
    return await execute_db(_chk)
...
async def db_add_reply(keyword: str, reply: str):
    await execute_db(lambda c: c.execute(
        "INSERT OR REPLACE INTO auto_replies (chat_id, keyword, reply, created_at) VALUES (0,?,?,?)",
        (keyword, reply, utc_now_iso())) or c.commit())
...
async def db_get_auto_reply_settings(chat_id: int) -> dict:
    async def _g(conn):
        ...
        row = await cur.fetchone()
        if row:
            return {'enabled': bool(row[1]) if len(row) > 1 else False,
                    'only_admins': bool(row[2]) if len(row) > 2 else False,
                    'ignore_bots': bool(row[3]) if len(row) > 3 else True}
        return {'enabled': False, 'only_admins': False, 'ignore_bots': True}
    return await execute_db(_g)

# ===================================================================
# 17. دوال الإعدادات العامة
# ===================================================================
...
async def db_set_setting(key: str, value: str):
    await execute_db(lambda c: c.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?,?)",
                                         (key, value)) or c.commit())

async def db_get_updates_channel() -> Optional[str]: return await db_get_setting('updates_channel')
async def db_get_force_subscribe_channel() -> Optional[str]: return await db_get_setting('force_subscribe_channel')
...
async def db_get_auto_backup() -> bool:
    ...
    return v == '1'
async def db_get_last_backup_time() -> Optional[str]: return await db_get_setting('last_backup')

async def db_get_users_needing_reminder():
    async def _g(conn):
        cur = await conn.execute("""
            SELECT u.user_id, u.subscription_end,
                   COALESCE(r.reminder_days_before, 3) as rdb,
                   COALESCE(r.notification_lang, 'ar') as nl,
                   COALESCE(r.last_reminder_sent, 0) as lrs
...
async def db_update_last_reminder_sent(user_id: int, reminder_type: str):
    await execute_db(lambda c: c.execute(
        "INSERT OR REPLACE INTO user_reminder_settings (user_id, last_reminder_sent) VALUES (?,?)",
        (user_id, utc_now_iso())) or c.commit())
...
async def db_get_due_scheduled_posts(now: datetime, limit: int = 50):
    return await execute_db(lambda c: c.execute(
        ...
        "WHERE publish_time <= ? AND fail_count < 5 ORDER BY publish_time ASC LIMIT ?",
        (now.isoformat(), limit)) or c.fetchall())
...
async def db_get_user_unpublished_posts(user_id: int) -> int:
    async def _c(conn):
        cur = await conn.execute(
            ...
            "WHERE uc.user_id=? AND p.published=0 AND uc.banned=0", (user_id,))
        return (await cur.fetchone())[0]
    return await execute_db(_c)
...
async def db_get_user_total_posts(user_id: int) -> int:
    async def _c(conn):
        cur = await conn.execute(
            "SELECT COUNT(*) FROM posts p JOIN user_channels uc ON p.channel_db_id=uc.id "
            "WHERE uc.user_id=? AND uc.banned=0", (user_id,))
        return (await cur.fetchone())[0]
    return await execute_db(_c)
...
async def db_get_channel_stats(channel_db_id: int) -> dict:
    async def _s(conn):
        ...
        return {'total_posts': total, 'published_posts': published,
                'unpublished_posts': total - published, 'total_views': 0, 'avg_views': 0}
    return await execute_db(_s)
...
async def db_save_schedule(channel_db_id: int, schedule_type: str, **kwargs):
    await execute_db(lambda c: c.execute(
        "INSERT OR REPLACE INTO schedule (channel_db_id, schedule_type, interval_minutes, interval_hours, "
        "interval_days, days_of_week, specific_dates, publish_time, cron_expression, next_publish_date) "
        "VALUES (?,?,?,?,?,?,?,?,?,NULL)",
        (channel_db_id, schedule_type,
         kwargs.get('interval_minutes', 12), kwargs.get('interval_hours', 0),
         kwargs.get('interval_days', 0), kwargs.get('days_of_week', '[]'),
         kwargs.get('specific_dates', '[]'), kwargs.get('publish_time', '00:00'),
         kwargs.get('cron_expression'))))

async def db_get_schedule(channel_db_id: int):
    async def _g(conn):
        cur = await conn.execute(
            "SELECT schedule_type, interval_minutes, interval_hours, interval_days, days_of_week, "
            "specific_dates, publish_time, cron_expression, next_publish_date FROM schedule WHERE channel_db_id=?",
            (channel_db_id,))
        row = await cur.fetchone()
        if row:
            return {'type': row[0] or 'interval_minutes', 'interval_minutes': row[1] or 12,
                    'interval_hours': row[2] or 0, 'interval_days': row[3] or 0,
                    'days_of_week': row[4] or '[]', 'specific_dates': row[5] or '[]',
                    'publish_time': row[6] or '00:00', 'cron_expression': row[7],
...
async def db_set_next_publish_date(channel_db_id: int, next_date: datetime):
    await execute_db(lambda c: c.execute("UPDATE schedule SET next_publish_date=? WHERE channel_db_id=?",
                                         (next_date.isoformat() if next_date else None, channel_db_id)) or c.commit())
...
async def db_set_last_publish(channel_db_id: int, publish_time: datetime):
    await execute_db(lambda c: c.execute(
        ...
        (channel_db_id, publish_time.isoformat())) or c.commit())

async def db_set_publish_time(channel_db_id: int, time_str: str):
    await execute_db(lambda c: c.execute(
        "UPDATE schedule SET publish_time=?, next_publish_date=NULL WHERE channel_db_id=?",
        (time_str, channel_db_id)) or c.commit())
...
async def db_update_next_publish_date(channel_db_id: int):
    async def _u(conn):
        ...
        cur = await conn.execute(
            "SELECT schedule_type, interval_minutes, interval_hours, interval_days FROM schedule WHERE channel_db_id=?",
            (channel_db_id,))
        row = await cur.fetchone()
        if not row:
            return
        ...
        if st == 'interval_minutes':
            nd = last_time + timedelta(minutes=row[1] or 12)
        elif st == 'interval_hours':
            nd = last_time + timedelta(hours=row[2] or 1)
        elif st == 'interval_days':
            nd = last_time + timedelta(days=row[3] or 1)
        else:
            nd = last_time + timedelta(minutes=12)
        if nd <= utc_now():
            while nd <= utc_now():
                if st == 'interval_minutes':
                    nd += timedelta(minutes=row[1] or 12)
                elif st == 'interval_hours':
                    nd += timedelta(hours=row[2] or 1)
                elif st == 'interval_days':
                    nd += timedelta(days=row[3] or 1)
                else:
                    nd += timedelta(minutes=12)
        await conn.execute("UPDATE schedule SET next_publish_date=? WHERE channel_db_id=?",
                           (nd.isoformat(), channel_db_id))
        await conn.commit()
    await execute_db(_u)
...
async def db_add_scheduled_post(chat_id: int, text: str, publish_time: datetime):
    await execute_db(lambda c: c.execute(
        ...
        (chat_id, text, publish_time.isoformat())) or c.commit())

async def db_register_hidden_owner_group(chat_id: int, owner_id: int) -> bool:
    await execute_db(lambda c: c.execute(
        "INSERT OR REPLACE INTO hidden_owner_groups (chat_id, owner_id) VALUES (?,?)",
        (chat_id, owner_id)) or c.commit())
    return True
...
async def db_remove_hidden_admin(chat_id: int, admin_id: int) -> bool:
    await execute_db(lambda c: c.execute("DELETE FROM hidden_admins WHERE chat_id=? AND admin_id=?",
                                         (chat_id, admin_id)) or c.commit())
    invalidate_auth_cache(chat_id, admin_id)
    return True

async def db_get_hidden_admins(chat_id: int):
    return await execute_db(lambda c: c.execute(
        "SELECT admin_id, added_by, added_at FROM hidden_admins WHERE chat_id=?", (chat_id,)) or c.fetchall())
        ...
        if context.args and context.args[0].startswith('ref_'):
            ...
            if referrer_id and referrer_id != user_id:
                if await db_add_referral(referrer_id, user_id):
                    ...
                    try:
                        await context.bot.send_message(referrer_id, f"🎉 مستخدم جديد عبر رابطك!\n🎁 مكافأتك: {reward} يوم")
                    except:
                        pass
            ...
            if admins:
                for a in admins:
                    if a.status == 'creator':
                        ...
                        break
                if real_id == user_id:
                    real_id = admins[0].user.id
        except:
            pass
    is_admin = await is_currently_admin_in_group(context.bot, chat_id, real_id)
    if is_admin:
...
async def register_hidden_owner_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if member.status not in ['administrator', 'creator']:
        await safe_send_markdown(context.bot, user_id, "🔒 للمشرفين فقط")
        return
...
def _build_security_text(settings: dict) -> str:
    ...
    lines = [
        ...
        f"⚖️ العقوبة: {settings.get('auto_penalty', 'لا شيء')}",
        "━━━━━━━━━━━━━━━━━━━━━━",
        "📌 اختر الإعداد:"
    ]
    return "\n".join(lines)
...
async def panel_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔒 قفل" if not locked else "🔓 فتح",
                              callback_data=f"panel:lock:{chat_id}" if not locked else f"panel:unlock:{chat_id}"),
         InlineKeyboardButton("🛠️ متقدم", callback_data=f"advanced_actions:{chat_id}")],
...
async def trial_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await db_has_active_subscription(user_id):
        await safe_send_markdown(context.bot, user_id, "✅ لديك اشتراك")
        return
...
async def subscribe_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await db_has_active_subscription(user_id):
        ...
        await safe_send_markdown(context.bot, user_id, f"✅ متبقي {d} يوم")
        return
    kb = InlineKeyboardMarkup([
...
async def support_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_markdown(context.bot, update.effective_user.id, "📞 أرسل رسالتك وسنرد عليك")
...
async def top_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not users:
        await safe_send_markdown(context.bot, update.effective_user.id, "📭 لا يوجد")
        return
    text = "🏆 **أفضل 10**\n"
...
async def set_rules_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await safe_send_markdown(context.bot, chat_id, "📝 أرسل: /set_rules النص")
        return
    rules = " ".join(args)
    await execute_db(lambda c: c.execute(
...
async def declare_winner_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await safe_send_markdown(context.bot, user_id, "/declare_winner id winner")
        return
    try:
        cid = int(args[0])
        wid = int(args[1])
        ...
        try:
            await context.bot.send_message(wid, f"🏆 فزت في {contest['title']}!")
        except:
            pass
    except:
...
async def contests_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not contests:
        await safe_send_markdown(context.bot, user_id, "📭 لا توجد")
        return
    text = "🏆 **المسابقات**\n"
    kb = []
    for c in contests:
        ...
        try:
            dl = (datetime.fromisoformat(end_date) - utc_now()).days
            tl = f"⏳ {dl} يوم" if dl > 0 else "🔴 انتهت"
        except:
            tl = "📅"
            dl = 0
        ...
        text += f"📌 {title}\n🎁 {prize}\n👥 {participants}\n{tl}\n\n"
        if not p and dl > 0:
            kb.append([InlineKeyboardButton(f"شارك في {title[:20]}", callback_data=f"contest_join:{cid}")])
...
async def handle_moderation_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        try:
            ...
            await safe_send_markdown(context.bot, chat_id, "❌ معرف غير صالح")
            return
    else:
        await safe_send_markdown(context.bot, chat_id, "❌ ارد على رسالة أو أرسل معرف")
        return
...
async def my_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channels = await db_get_channels(user_id)
    if not channels:
        await _safe_edit(query, "📭 لا توجد قنوات")
        return
    kb = []
...
async def add_15_posts_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active = context.user_data.get('active_channel') or await db_get_active_channel(user_id)
    if not active:
        await _safe_edit(query, "⚠️ اختر قناة")
        return
    unpub = await db_unpublished_count(active)
    if unpub >= MAX_UNPUBLISHED_POSTS:
        await _safe_edit(query, "⚠️ الحد الأقصى")
        return
...
async def publish_one_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    if not await db_has_active_subscription(user_id) and not await db_has_used_trial(user_id):
        await _safe_edit(query, "⚠️ اشتراك منتهٍ")
        return
    active = context.user_data.get('active_channel') or await db_get_active_channel(user_id)
    if not active:
        await _safe_edit(query, "⚠️ اختر قناة")
    ...
    if not post:
        await _safe_edit(query, "📭 لا توجد منشورات")
        return
    ch_info = await db_get_channel_info(active)
    if not ch_info:
        return
    ...
    try:
        if post['media_type'] == 'photo' and post['media_file_id']:
            await context.bot.send_photo(channel_id, post['media_file_id'],
                                         caption=post['text'][:1024] if post['text'] else None)
        elif post['media_type'] == 'video' and post['media_file_id']:
            await context.bot.send_video(channel_id, post['media_file_id'],
                                         caption=post['text'][:1024] if post['text'] else None)
        elif post['media_type'] == 'document' and post['media_file_id']:
            await context.bot.send_document(channel_id, post['media_file_id'],
                                            caption=post['text'][:1024] if post['text'] else None)
        else:
...
async def my_posts_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not posts:
        await _safe_edit(query, "📭 لا توجد")
        return
    text = "📋 **منشوراتي**\n"
    kb = []
...
async def publish_all_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ch_db_id, ch_tele_id, ch_name, banned in channels:
        ...
        try:
            if post['media_type'] == 'photo' and post['media_file_id']:
                await context.bot.send_photo(ch_tele_id, post['media_file_id'],
                                             caption=post['text'][:1024] if post['text'] else None)
            else:
                await context.bot.send_message(ch_tele_id, post['text'][:4096] if post['text'] else ".")
            await db_mark_published(post['id'])
...
async def my_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not valid:
        ...
        await _safe_edit(query, "📭 لا توجد", reply_markup=kb)
        return
    kb = []
...
async def group_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_authorized_in_group(context.bot, chat_id, uid):
        return
    settings = await db_get_security_settings(chat_id)
    await _safe_edit(query, _build_security_text(settings), reply_markup=security_keyboard(chat_id))
...
# ===================================================================
# 23. معالج الأمان - زر التبديل (المصحح)
# ===================================================================
...
async def group_action_log_callback(update, context):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"advanced_actions:{chat_id}")]])
    await _safe_edit(query, log_text, reply_markup=kb)
...
# ===================================================================
# 25. معالج الأزرار العام (مع جميع الأزرار مربوطة)
# ===================================================================
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # الجدولة
        if data.startswith("schedule:menu:"): return await schedule_menu_callback(update, context)
        if data.startswith("schedule:set_interval_minutes:"): return await set_interval_minutes_callback(update, context)
        if data.startswith("schedule:set_interval_hours:"): return await set_interval_hours_callback(update, context)
        if data.startswith("schedule:set_interval_days:"): return await set_interval_days_callback(update, context)
        if data.startswith("schedule:set_days:"): return await set_days_callback(update, context)
        if data.startswith("schedule:set_dates:"): return await set_dates_callback(update, context)
        if data.startswith("schedule:set_publish_time:"): return await set_publish_time_callback(update, context)
        if data.startswith("schedule:set_cron:"): return await set_cron_callback(update, context)
        if data.startswith("schedule:day_select:"): return await day_select_callback(update, context)
        ...
        if data.startswith("security:") and len(data.split(":")) >= 3:
            action = data.split(":")[1]
            if action in ["links", "mentions", "slow_mode", "delete_videos", "delete_audio",
        ...
        if data.startswith("referral:copy_link:"): return await referral_copy_link_callback(update, context)
        ...
        if data.startswith("reminder:lang:"): return await reminder_lang_callback(update, context)
        ...
        if data.startswith("translation:set:"): return await translation_set_callback(update, context)

        # المسابقات
        if data == CallbackData.CONTESTS_MENU: return await contests_command_handler(update, context)
        if data.startswith("contest_join:"): return await contest_join_callback(update, context)
        ...
        # الردود التلقائية
        if data.startswith("auto_reply_menu:"): return await auto_reply_menu_callback(update, context)
        if data.startswith("auto_reply_toggle:"): return await auto_reply_toggle_callback(update, context)
        if data.startswith("auto_reply_admins:"): return await auto_reply_admins_callback(update, context)
        if data.startswith("auto_reply_reset:"): return await auto_reply_reset_callback(update, context)
        if data.startswith("auto_reply_confirm_reset:"): return await auto_reply_confirm_reset_callback(update, context)
        if data.startswith("auto_reply_stats:"): return await auto_reply_stats_callback(update, context)
        if data.startswith("user_auto_reply_toggle:"): return await user_auto_reply_toggle_callback(update, context)

        # التحقق من الاشتراك
        if data == CallbackData.CHECK_SUBSCRIBE: return await check_subscribe_callback_handler(update, context)
        ...
        await _answer_query(query)
        logger.warning(f"بيانات غير معروفة: {data}")
    except Exception as e:
        error_id = log_error(e)
        try:
            await _answer_query(query)
...
# ===================================================================
# 26. الدوال المساعدة المتبقية (الإحالات، التذكيرات، الترجمة، NSFW، الاشتراك)
# ===================================================================
...
async def referral_list_callback(update, context):
    ...
    async def _g(conn):
        ...
        return await cur.fetchall()
    refs = await execute_db(_g)
    if not refs:
        await _safe_edit(query, "📭 لا توجد")
        return
    text = "📋 المحالون:\n"
    for i, (rid, created) in enumerate(refs, 1):
        text += f"{i}. `{rid}` - {created[:10]}\n"
    await _safe_edit(query, text)
...
async def nsfw_threshold_set_callback(update, context):
    query = update.callback_query; await query.answer()
    context.user_data['state'] = UserState.WAITING_NSFW_THRESHOLD
    await _safe_edit(query, "📊 أرسل النسبة (0-100):")

async def auto_reply_menu_callback(update, context):
    query = update.callback_query; await query.answer()
    chat_id = int(query.data.split(":")[-1])
...
async def buy_subscription_1_callback(update, context):
    query = update.callback_query; await query.answer()
    try:
...
async def buy_subscription_2_callback(update, context):
    query = update.callback_query; await query.answer()
    try:
        await context.bot.send_invoice(update.effective_user.id, "يومين", "اشتراك يومين", "sub_2", "", "XTR", [LabeledPrice("يومين", 9)])
    except:
        pass

async def buy_subscription_30_callback(update, context):
    query = update.callback_query; await query.answer()
    try:
        await context.bot.send_invoice(update.effective_user.id, "شهر", "اشتراك شهر", "sub_30", "", "XTR", [LabeledPrice("شهر", 50)])
    except:
        pass

async def buy_subscription_90_callback(update, context):
    query = update.callback_query; await query.answer()
    try:
        await context.bot.send_invoice(update.effective_user.id, "3 أشهر", "اشتراك 3 أشهر", "sub_90", "", "XTR", [LabeledPrice("3 أشهر", 120)])
    except:
        pass
...
async def set_interval_minutes_callback(update, context):
    ...
    context.user_data['schedule_ch_id'] = ch_db_id
    await _safe_edit(query, "⏱️ أرسل الدقائق (1-1440):")
...
async def set_interval_hours_callback(update, context):
    ...
    context.user_data['schedule_ch_id'] = ch_db_id
    await _safe_edit(query, "⏱️ أرسل الساعات (1-168):")
...
async def set_interval_days_callback(update, context):
    ...
    context.user_data['schedule_ch_id'] = ch_db_id
    await _safe_edit(query, "⏱️ أرسل الأيام (1-365):")
...
async def filter_messages_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if settings.get('delete_banned_words') and text:
        ...
        if word:
            await delete_and_penalize(update, context, "🚫 كلمة محظورة!")
            return
    delete_media = False
    msg = update.message
    ...
    if settings.get('antiflood_enabled') and await db_check_antiflood(chat_id, user_id):
        ...
        await apply_penalty_with_duration(context.bot, chat_id, user_id,
                                          settings.get('antiflood_penalty', 'mute'), 60)
        return
    ...
    if text:
        ...
        if len(text) > 3:
            ...
            async def _save(conn):
                await conn.execute("INSERT INTO sentiment_history (user_id, chat_id, text, sentiment, score, created_at) VALUES (?,?,?,?,?,?)",
                                   (user_id, chat_id, text[:200], sentiment['sentiment'], sentiment['score'], utc_now_iso()))
                await conn.commit()
            try: await execute_db(_save)
            except: pass
    ...
    if state == UserState.WAITING_CHANNEL_ID:
        ...
        try:
            ...
            try:
                ...
                await safe_send_markdown(context.bot, user_id, "❌ لا يمكن الوصول")
                return
        ...
        msg = update.message
        if msg.photo:
            media_type = 'photo'; media_file_id = msg.photo[-1].file_id
        elif msg.video:
            media_type = 'video'; media_file_id = msg.video.file_id
        elif msg.document:
            media_type = 'document'; media_file_id = msg.document.file_id
        elif msg.audio:
            media_type = 'audio'; media_file_id = msg.audio.file_id
        elif msg.voice:
            media_type = 'voice'; media_file_id = msg.voice.file_id
        elif msg.animation:
            media_type = 'animation'; media_file_id = msg.animation.file_id
        elif msg.text:
            media_type = 'text'
        else:
        ...
        try:
            ...
            if end_date > mecca_now():
                cid = await db_create_contest(user_id,
                                              context.user_data.pop('contest_title', ''),
                                              context.user_data.pop('contest_description', ''),
                                              context.user_data.pop('contest_prize', ''),
                                              mecca_to_utc(end_date))
    ...
    while True:
        try:
            ...
            for ch_db_id, ch_tele_id, user_id in rows:
                ...
                try:
                    if post['media_type'] == 'photo' and post['media_file_id']:
                        await bot.send_photo(ch_tele_id, post['media_file_id'],
                                             caption=post['text'][:1024] if post['text'] else None)
                    else:
...
async def cleanup_expired_sessions_improved():
    while True:
        ...
        try:
            await execute_db(lambda c: c.execute(
                "DELETE FROM sentiment_history WHERE created_at < ?",
                ((utc_now() - timedelta(days=90)).isoformat(),)) or c.commit())
        except:
            pass
        ...
        try:
            ...
            for cid, title, prize in await execute_db(_g):
                ...
                if wid:
                    await db_set_contest_winner(cid, wid)
                    try:
                        await bot.send_message(wid, f"🏆 فزت في {title}!")
                    except:
                        pass
        except:
            pass
...
async def main():
    ...
    if words:
        for w in words:
            try:
                await db_add_banned_word(w, -1, PRIMARY_OWNER_ID)
            except:
                pass
        await rebuild_banned_patterns()
    ...
    if USE_PROXY:
        request = HTTPXRequest(proxy_url=PROXY_URL, read_timeout=60, write_timeout=30,
                               connect_timeout=30, connection_pool_size=MAX_CONNECTIONS)
    else:
        request = HTTPXRequest(read_timeout=60, write_timeout=30,
                               connect_timeout=30, connection_pool_size=MAX_CONNECTIONS)
    ...
    application.add_handler(MessageHandler(
        ...
        filter_messages_handler), group=1)
    application.add_handler(MessageHandler(
    ...
    try:
        await application.bot.set_my_commands([
            ...
            BotCommand("panel", "لوحة تحكم"), BotCommand("lock", "قفل"),
            BotCommand("unlock", "فتح"), BotCommand("ban", "حظر"),
            BotCommand("mute", "كتم"), BotCommand("warn", "تحذير"),
