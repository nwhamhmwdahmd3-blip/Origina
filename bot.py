...
"""
🌿 Relax Manager – النسخة النهائية الكاملة v7.0.0
================================================================================
...
from typing import Optional, List, Dict, Tuple, Any, Union, Callable, Awaitable, Set
...
LOCALES = {
    'ar': {
        ...
        'error_message_too_long': "⚠️ الرسالة طويلة جداً، تم تقصيرها",
        'error_not_enough_rights': "⚠️ البوت لا يملك صلاحيات كافية",
        'error_user_admin': "⚠️ لا يمكن معاملة مشرف",
        'error_cant_demote': "⚠️ لا يمكن خفض صلاحيات المشرف",
        'no_invoices': "📭 لا توجد فواتير",
        'payment_error': "❌ خطأ في الدفع، يرجى المحاولة لاحقاً",
        'enter_reply': "✏️ أرسل الرد:",
        'auto_reply_deleted': "✅ تم حذف الرد لـ '{keyword}'",
        'auto_reply_not_found': "❌ لا يوجد رد لـ '{keyword}'",
        'referral_link_copied': "🔗 رابط الإحالة: {link}",
    ...
    'en': {
        ...
        'error_message_too_long': "⚠️ Message too long, truncated",
        'error_not_enough_rights': "⚠️ Bot doesn't have enough rights",
        'error_user_admin': "⚠️ Cannot act on admin",
        'error_cant_demote': "⚠️ Cannot demote admin",
        'no_invoices': "📭 No invoices",
        'payment_error': "❌ Payment error, please try later",
        'enter_reply': "✏️ Send the reply:",
        'auto_reply_deleted': "✅ Deleted reply for '{keyword}'",
        'auto_reply_not_found': "❌ No reply found for '{keyword}'",
        'referral_link_copied': "🔗 Referral link: {link}",
...
# =====================================================================
# 7. قاعدة البيانات (مع جميع الجداول)
# =====================================================================
class Database:
    _instance = None
    _lock = asyncio.Lock()
    ...
    async def _create_indexes(self, conn) -> None:
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_user_groups_link ON user_groups_link(user_id)")
        await conn.commit()
...
# =====================================================================
# 8. المستودعات (Repositories) - جميع المستودعات كاملة
# =====================================================================
...
class UserRepository:
    ...
    async def register(user_id: int) -> bool:
        try:
            ...
            await DB.execute(
                "INSERT INTO users (user_id, referral_code, created_at, updated_at) VALUES (?,?,?,?)",
    ...
    async def get_stats() -> Dict[str, int]:
        try:
            ...
            return {'users': 0, 'banned': 0, 'posts': 0, 'groups': 0, 'channels': 0}
...
class ChannelRepository:
    ...
    async def add(user_id: int, channel_id: str, channel_name: str) -> Optional[int]:
        try:
            ...
            if row:
                return None
            safe_name = TextUtils.sanitize(channel_name, max_len=255)
            result = await DB.fetchone(
                "INSERT INTO user_channels (user_id, channel_id, channel_name, created_at) VALUES (?,?,?,?) RETURNING id",
                (user_id, channel_id, safe_name, TimeUtils.utc_iso())
            )
            if not result:
                return None
            ch_db_id = result[0]
            ...
            log_error(e, {'user_id': user_id, 'channel_id': channel_id})
            return None
    ...
    @staticmethod
    async def ban(channel_db_id: int) -> None:
        await DB.execute("UPDATE user_channels SET banned=1 WHERE id=?", (channel_db_id,))

    @staticmethod
    async def unban(channel_db_id: int) -> None:
        await DB.execute("UPDATE user_channels SET banned=0 WHERE id=?", (channel_db_id,))
...
class PostRepository:
    ...
    @staticmethod
    async def delete_all(channel_db_id: int) -> None:
        await DB.execute("DELETE FROM posts WHERE channel_db_id=?", (channel_db_id,))

# 8.4 ScheduleRepository
...
class GroupRepository:
        ...
        try:
            ...
            async with DB._get_connection() as conn:
                async with conn.execute(
                    ...
                    "FROM bot_groups bg "
                    "INNER JOIN hidden_owner_groups h ON bg.chat_id = h.chat_id "
                    "WHERE h.owner_id = ?",
                    (user_id,)
                ) as cur:
                ...
                async with conn.execute(
                    "SELECT DISTINCT bg.chat_id, bg.chat_name, bg.username, bg.banned "
                    "FROM bot_groups bg "
                    "INNER JOIN hidden_admins h ON bg.chat_id = h.chat_id "
                    "WHERE h.admin_id = ?",
                    (user_id,)
                ) as cur:
                    rows = await cur.fetchall()
                ...
                async with conn.execute(
                    "SELECT DISTINCT bg.chat_id, bg.chat_name, bg.username, bg.banned "
                    "FROM bot_groups bg "
                    "INNER JOIN group_admins g ON bg.chat_id = g.chat_id "
                    "WHERE g.user_id = ?",
                    (user_id,)
                ) as cur:
                    rows = await cur.fetchall()
                ...
                async with conn.execute(
                    "SELECT DISTINCT bg.chat_id, bg.chat_name, bg.username, bg.banned "
                    "FROM bot_groups bg "
                    "INNER JOIN user_groups_link l ON bg.chat_id = l.chat_id "
                    "WHERE l.user_id = ?",
                    (user_id,)
                ) as cur:
                    rows = await cur.fetchall()
    ...
    async def sync_admins(chat_id: int, bot) -> int:
        try:
            ...
            log_error(e, {'chat_id': chat_id})
            return 0

    @staticmethod
    async def link_user_to_group(user_id: int, chat_id: int) -> None:
        try:
            await DB.execute(
            ...
            log_error(e, {'user_id': user_id, 'chat_id': chat_id})

    @staticmethod
    async def unlink_user_from_group(user_id: int, chat_id: int) -> None:
        try:
            await DB.execute(
                "DELETE FROM user_groups_link WHERE user_id=? AND chat_id=?",
                (user_id, chat_id)
            )
        except Exception as e:
            log_error(e, {'user_id': user_id, 'chat_id': chat_id})
    ...
    @staticmethod
    async def is_user_linked(user_id: int, chat_id: int) -> bool:
        try:
            row = await DB.fetchone(
                "SELECT 1 FROM user_groups_link WHERE user_id=? AND chat_id=?",
                (user_id, chat_id)
            )
...
class SecurityRepository:
    ...
    def invalidate_cache(cls, chat_id: int = None):
        """مسح الكاش لمجموعة محددة أو مسح الكاش كله"""
        if chat_id:
...
class AutoReplyRepository:
    ...
    async def set_enabled(cls, chat_id: int, enabled: bool) -> None:
        try:
            await DB.execute(
                "INSERT OR REPLACE INTO auto_reply_settings (chat_id, enabled, updated_at) VALUES (?,?,?)",
    ...
    async def set_only_admins(cls, chat_id: int, only_admins: bool) -> None:
        try:
            await DB.execute(
                "UPDATE auto_reply_settings SET only_admins=?, updated_at=? WHERE chat_id=?",
    ...
    async def add_reply(cls, chat_id: int, keyword: str, reply: str, reply_type: str = 'text',
        ...
        try:
            ...
            cls._cache.pop(chat_id, None)
            _auto_reply_cache.invalidate(f"{chat_id}:{keyword.lower()}")
        except:
            pass
    ...
    async def get_reply(cls, keyword: str, chat_id: int = 0) -> Optional[dict]:
        ...
        try:
            ...
            try:
                ...
                buttons = None
            reply_data = {'reply': row[0], 'type': row[1], 'media_id': row[2], 'buttons': buttons}
        ...
        try:
            if chat_id == -1:
                return await DB.fetchall(
                    "SELECT keyword, usage_count FROM auto_replies WHERE is_active=1 ORDER BY usage_count DESC LIMIT ?",
                    (limit,)
                )
            else:
                return await DB.fetchall(
...
class ReferralRepository:
    ...
    async def get_stats(user_id: int) -> dict:
        try:
            ...
            return {'total': 0, 'count': 0, 'total_days': 0, 'claimed': 0, 'available': 0}
...
class BotAdminRepository:
    ...
    async def get_all_admins() -> List[int]:
        try:
            rows = await DB.fetchall("SELECT user_id FROM bot_admins")
            return [row[0] for row in rows]
        except:
            return []

# =====================================================================
# 9. دوال مساعدة إضافية للردود التلقائية والمشاعر
# =====================================================================
        ...
        if member.status in ['administrator', 'creator']:
            ...
            if row:
                ...
                if row2:
                    ...
                    linked = await GroupRepository.is_user_linked(user_id, chat_id)
                    if linked:
                        authorized = True
    except:
        authorized = False
...
# =====================================================================
class CB:
    MAIN = "main"; BACK = "back"; CANCEL = "cancel"
    CH_ADD = "ch_add"; CH_LIST = "ch_list"; CH_DEL = "ch_del:"; CH_SEL = "ch_sel:"
    POST_ADD = "post_add"; POST_PUB = "post_pub"; POST_LIST = "post_list"
    POST_REC = "post_rec"; POST_DEL = "post_del:"; POST_CLEAR = "post_clear:"
    PUB_ALL = "pub_all"; STATS_PEND = "stats_pend"; STATS_FULL = "stats_full"
    GROUPS = "groups"; GRP_SET = "grp_set:"
    SETTINGS = "settings"; TOGGLE_AUTO = "toggle_auto"; TOGGLE_REC = "toggle_rec"
    SCHEDULE = "schedule:"; SCHED_MIN = "sched_min:"; SCHED_HOUR = "sched_hour:"
    SCHED_DAY = "sched_day:"; SCHED_TIME = "sched_time:"
    SEC_BANNED = "sec_banned:"; SEC_CLOSE = "sec_close"
    SEC_ENABLE_ALL = "sec_enable_all:"; SEC_DISABLE_ALL = "sec_disable_all:"
    SEC_DEL_PEN = "sec_del_pen:"
    BAN_ADD = "ban_add:"; BAN_LIST = "ban_list:"; BAN_REM = "ban_rem:"
    PENALTY = "penalty:"; PEN_KICK = "pen_kick:"; PEN_BAN = "pen_ban:"
    PEN_MUTE = "pen_mute:"; PEN_WARN = "pen_warn:"; PEN_RESTRICT = "pen_restrict:"
    PEN_NONE = "pen_none:"
    ADV_ACT = "adv_act:"; ACT_BAN = "act_ban:"; ACT_MUTE = "act_mute:"
    ACT_WARN = "act_warn:"; ACT_KICK = "act_kick:"; ACT_RESTRICT = "act_restrict:"
    ACT_PIN = "act_pin:"; ACT_LOG = "act_log:"; ACT_UNBAN = "act_unban:"
    MUTE_DUR = "mute_dur:"
    PANEL_LOCK = "panel_lock:"; PANEL_UNLOCK = "panel_unlock:"; PANEL_CLOSE = "panel_close"
    HELP = "help"; SUPPORT = "support"; SUPPORT_TICKET = "support_ticket"
    TRIAL = "trial"; SUBSCRIBE = "subscribe"; BUY_SUB = "buy_sub:"
    DEVELOPER = "developer"; UPDATES = "updates"
    REFERRAL = "referral"; REF_CLAIM = "ref_claim"; REF_LIST = "ref_list"
    REMINDER = "reminder"; REM_TOGGLE_SUB = "rem_sub"; REM_TOGGLE_DAILY = "rem_daily"
    REM_TOGGLE_WEEKLY = "rem_weekly"; REM_SET_DAYS = "rem_days"; REM_SET_LANG = "rem_lang"
    REM_LANG = "rem_lang:"
    TRANSLATION = "translation"; TRANS_OFF = "trans_off"; TRANS_SET = "trans_set:"
    CONTESTS = "contests"; CONTEST_JOIN = "contest_join:"; CONTEST_WINNERS = "contest_winners"
    ADMIN = "admin"; ADMIN_USERS = "admin_users"; ADMIN_BANNED = "admin_banned"
    ADMIN_UNBAN_ALL = "admin_unban_all"; ADMIN_CHANNELS = "admin_channels"
    ADMIN_BANNED_CH = "admin_banned_ch"; ADMIN_ACTIVATE_CH = "admin_activate_ch"
    ADMIN_GROUPS = "admin_groups"; ADMIN_BANNED_GR = "admin_banned_gr"
    ADMIN_UNBAN_GR = "admin_unban_gr"; ADMIN_ADD_ADMIN = "admin_add_admin"
    ADMIN_REM_ADMIN = "admin_rem_admin"; ADMIN_RAM = "admin_ram"
    ADMIN_STATS = "admin_stats"; ADMIN_METRICS = "admin_metrics"
    ADMIN_BACKUP = "admin_backup"; ADMIN_RESTORE = "admin_restore"
    ADMIN_RESTORE_SEL = "admin_restore_sel:"
    ADMIN_SEND_UPDATE = "admin_send_update"; ADMIN_SET_UPDATE_CH = "admin_set_update_ch"
    ADMIN_SHOW_UPDATE = "admin_show_update"; ADMIN_FORCE_SUB = "admin_force_sub"
    ADMIN_SET_FORCE = "admin_set_force"; ADMIN_BROADCAST = "admin_broadcast"
    ADMIN_CONFIRM_BROADCAST = "admin_confirm_broadcast"; ADMIN_TICKETS = "admin_tickets"
    ADMIN_DEL_TICKETS = "admin_del_tickets"; ADMIN_CONFIRM_DEL_TICKETS = "admin_confirm_del_tickets"
    ADMIN_LOG_CH = "admin_log_ch"; ADMIN_SET_LOG_CH = "admin_set_log_ch"
    ADMIN_REPLIES = "admin_replies"; ADMIN_ADD_REPLY = "admin_add_reply"
    ADMIN_LIST_REPLIES = "admin_list_replies"; ADMIN_DEL_REPLY = "admin_del_reply"
    ADMIN_BANNED_WORDS = "admin_banned_words"; ADMIN_ADD_BANNED = "admin_add_banned"
    ADMIN_LIST_BANNED = "admin_list_banned"; ADMIN_REM_BANNED = "admin_rem_banned"
    ADMIN_CREATE_CONTEST = "admin_create_contest"; ADMIN_DECLARE_WINNER = "admin_declare_winner"
    ADMIN_DEL_CONTEST = "admin_del_contest:"
    AUTO_REPLY_MENU = "auto_reply_menu:"; AUTO_REPLY_TOGGLE = "auto_reply_toggle:"
    AUTO_REPLY_ADMINS = "auto_reply_admins:"; AUTO_REPLY_RESET = "auto_reply_reset:"
    AUTO_REPLY_CONFIRM_RESET = "auto_reply_confirm_reset:"; AUTO_REPLY_STATS = "auto_reply_stats:"
    AUTO_REPLY_ADD = "auto_reply_add:"; AUTO_REPLY_DEL = "auto_reply_del:"
    AUTO_REPLY_LIST = "auto_reply_list:"; CHECK_SUB = "check_sub"
    PLANS = "plans"; INVOICES = "invoices"
    ADMIN_EXPORT_REPLIES = "admin_export_replies"; ADMIN_IMPORT_REPLIES = "admin_import_replies"
    ADMIN_REFRESH_CACHE = "admin_refresh_cache"; ADMIN_CONFIRM_IMPORT = "admin_confirm_import"
...
class KeyboardFactory:
    ...
    def admin(lang: str = 'ar') -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📥 استيراد من GitHub", callback_data=CB.ADMIN_IMPORT_GITHUB)],
...
class CommandHandlers:
    ...
    async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await UserRepository.update_username(user_id, username, first_name)
        args = context.args
    ...
    async def syncgroup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if is_admin:
            ...
            if user_id == CONFIG.ANONYMOUS_ADMIN_ID and user_id != real_user_id:
                await safe_send(context.bot, user_id, f"🔍 تم تسجيلك كمالك مخفي باستخدام معرفك الحقيقي: `{real_user_id}`")
            try:
            ...
            try:
                ...
                for admin in admins:
                    if admin.user.id != user_id:
                        try:
                            await safe_send(context.bot, admin.user.id,
    ...
    async def remove_hidden_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await GroupRepository.unlink_user_from_group(target, chat_id)
        invalidate_auth_cache(chat_id, target)
...
# =====================================================================
# 17. معالج الكولباك - CallbackHandlers
# =====================================================================
class CallbackHandlers:
    @staticmethod
    async def _build_security_message(settings: dict, lang: str, chat_id: int) -> Tuple[str, InlineKeyboardMarkup]:
        text = get_text(lang, 'security_text',
            ...
            auto_penalty=settings.get('auto_penalty', 'none'),
            delete_penalty=settings.get('delete_penalty', 'none'))
        keyboard = KeyboardFactory.security(chat_id, settings, lang)
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data == CB.CH_ADD:
            if not await UserRepository.has_active_subscription(user_id) and not await UserRepository.has_used_trial(user_id):
                await query.answer(get_text(lang, 'subscription_expired'), show_alert=True)
                return
        ...
        if data.startswith(CB.GRP_SET):
            ...
            if not await is_authorized_in_group(context.bot, chat_id, user_id):
                ...
                return
            settings = await SecurityRepository.get(chat_id)
            text, keyboard = await CallbackHandlers._build_security_message(settings, lang, chat_id)
        ...
        if data.startswith(CB.PANEL_LOCK):
            chat_id = int(data.split(":")[-1])
            await ChatLockRepository.set_lock(chat_id, True, user_id)
            await query.edit_message_text("🔒 تم القفل")
            return
        if data.startswith(CB.PANEL_UNLOCK):
            chat_id = int(data.split(":")[-1])
            await ChatLockRepository.set_lock(chat_id, False)
            await query.edit_message_text("🔓 تم الفتح")
            return
        ...
        if data == CB.ADMIN_REFRESH_CACHE:
            _auto_reply_cache.invalidate()
            SecurityRepository.invalidate_cache()
    ...
    async def _handle_security_callback(update, context, query, user_id, lang):
        ...
        try:
            ...
            await query.answer("❌ خطأ في معرف المجموعة")
            return
        ...
        if action in ["enable_all", "disable_all"]:
            fields = ['delete_videos', 'delete_audio', 'delete_animation', 'delete_service',
                      'delete_documents', 'delete_stickers', 'delete_forwarded', 'delete_polls',
                      'delete_games', 'delete_voice', 'delete_video_note']
            value = 1 if action == "enable_all" else 0
            for f in fields:
                await SecurityRepository.set(chat_id, **{f: value})
            SecurityRepository.invalidate_cache(chat_id)
        ...
        if action == "warn":
            ...
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("📝 تغيير عدد التحذيرات", callback_data=f"sec_warn_count:{chat_id}"),
                 InlineKeyboardButton("⚖️ تغيير العقوبة", callback_data=f"sec_warn_penalty:{chat_id}")],
        ...
        if action in field_map:
            ...
            await DB.execute(
                f"UPDATE group_security SET {col} = 1 - {col} WHERE chat_id = ?",
                (chat_id,)
            )
            SecurityRepository.invalidate_cache(chat_id)
            settings = await SecurityRepository.get(chat_id, force_refresh=True)
    ...
    async def _handle_admin_callback(update, context, query, user_id, lang):
        ...
        if data == CB.ADMIN_USERS:
            ...
            await query.edit_message_text(get_text(lang, 'admin_metrics',
...
# =====================================================================
# 18. معالج الرسائل - MessageHandlers
# =====================================================================
class MessageHandlers:
    ...
    async def handle_private(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if state == UserState.WAIT_CHANNEL:
            if not await UserRepository.has_active_subscription(user_id) and not await UserRepository.has_used_trial(user_id):
                await safe_send(context.bot, user_id, get_text(lang, 'subscription_expired'))
                StateManager.clear(user_id)
                return
            ...
            try:
                ...
                if "chat not found" in error_msg or "user not found" in error_msg:
                    await safe_send(context.bot, user_id, "❌ القناة غير موجودة. تأكد من المعرف.")
                else:
                    await safe_send(context.bot, user_id, f"❌ خطأ في جلب القناة: {str(e)[:100]}")
                StateManager.clear(user_id)
                return
            except Exception as e:
                log_error(e, {'user_id': user_id, 'channel_input': channel_input})
                await safe_send(context.bot, user_id, f"❌ خطأ غير متوقع: {str(e)[:100]}")
                StateManager.clear(user_id)
                return
            ...
            try:
                ...
                if "user not participant" in error_msg:
                    await safe_send(context.bot, user_id, "❌ البوت ليس عضواً في القناة. أضف البوت إلى القناة أولاً.")
                else:
                    await safe_send(context.bot, user_id, f"❌ فشل التحقق من صلاحيات البوت: {str(e)[:100]}")
                StateManager.clear(user_id)
                return
            except Exception as e:
                log_error(e, {'user_id': user_id, 'chat_id': chat.id})
                await safe_send(context.bot, user_id, f"❌ خطأ غير متوقع: {str(e)[:100]}")
                StateManager.clear(user_id)
                return
            if bot_member.status != 'administrator':
                await safe_send(context.bot, user_id, "❌ البوت ليس مشرفاً في القناة. الرجاء جعله مشرفاً مع صلاحية نشر الرسائل.")
                StateManager.clear(user_id)
                return
            if not hasattr(bot_member, 'can_post_messages') or not bot_member.can_post_messages:
                await safe_send(context.bot, user_id, "❌ البوت لا يملك صلاحية نشر الرسائل في القناة.")
                StateManager.clear(user_id)
                return
            try:
                ...
                if result:
                    ...
                    await safe_send(context.bot, user_id, get_text(lang, 'channel_exists'))
                    channels = await ChannelRepository.get_all(user_id)
                    if channels:
                        kb = []
                        for ch in channels:
                            ...
                            kb.append([InlineKeyboardButton(f"{st} {ch['channel_name']}", callback_data=f"{CB.CH_SEL}{ch['id']}")])
                        kb.append([InlineKeyboardButton(get_text(lang, 'back'), callback_data=CB.BACK)])
                        await safe_send(context.bot, user_id, "📡 **قنواتك الحالية**", reply_markup=InlineKeyboardMarkup(kb))
                    else:
                        await CommandHandlers.start(update, context)
            except Exception as e:
                log_error(e, {'user_id': user_id, 'channel_input': channel_input})
        ...
        if state == UserState.WAIT_REPLY_BUTTONS:
            chat_id_auto = context.user_data.get('auto_chat')
            keyword = context.user_data.get('auto_key')
            reply_text = context.user_data.get('auto_reply_text')
            if text == '/skip':
                if chat_id_auto is not None and keyword:
                    await AutoReplyRepository.add_reply(chat_id_auto, keyword, reply_text)
                    await safe_send(context.bot, user_id, get_text(lang, 'auto_reply_added', keyword=keyword))
                StateManager.clear(user_id)
                context.user_data.pop('auto_key', None)
                context.user_data.pop('auto_chat', None)
                context.user_data.pop('auto_reply_text', None)
                return
            buttons = []
            ...
            for line in lines:
                ...
                if not line:
                    continue
                if ' - ' in line:
                    btn_text, btn_value = line.split(' - ', 1)
                    btn_text = btn_text.strip()
                    btn_value = btn_value.strip()
                    if btn_value.startswith('http'):
                        buttons.append([{'text': btn_text, 'url': btn_value}])
                    else:
                        buttons.append([{'text': btn_text, 'callback_data': btn_value}])
            if chat_id_auto is not None and keyword:
                buttons_json = json.dumps(buttons) if buttons else None
                await AutoReplyRepository.add_reply(
                    chat_id_auto, keyword, reply_text,
                    reply_type='text',
                    reply_buttons=buttons_json
                )
                await safe_send(context.bot, user_id, f"✅ تم إضافة رد لـ '{keyword}' مع {len(buttons)} زر")
            StateManager.clear(user_id)
            context.user_data.pop('auto_key', None)
            context.user_data.pop('auto_chat', None)
            context.user_data.pop('auto_reply_text', None)
            ...
            try:
                ...
                if val >= 0:
                    ...
                    if chat_id_sec:
                        ...
                        SecurityRepository.invalidate_cache(chat_id_sec)
        ...
        if state == UserState.SUPPORT_MODE:
            ...
            admins = await BotAdminRepository.get_all_admins()
    ...
    async def handle_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            logger.warning(f"Auto-reply failed in group {chat_id}: {e}")

        if len(text) > 3:
            try:
                res = SENTIMENT.analyze(text)
                await save_sentiment(user_id, chat_id, text, res['sentiment'], res['score'])
            except:
                pass
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            await safe_send(context.bot, user_id, get_text(lang, 'payment_success', plan=plan['name'] if plan else "", days=plan['duration_days'] if plan else ""))
        else:
            await safe_send(context.bot, user_id, get_text(lang, 'payment_failed'))
    except Exception as e:
        log_error(e, {'user_id': user_id, 'payload': payload})
        await safe_send(context.bot, user_id, get_text(lang, 'payment_failed_generic'))
...
async def setup_unified_web_server(application, port: int):
    ...
    application.web_app.router.add_post(f"/{CONFIG.TOKEN}", webhook)
...
class BackgroundTasks:
        ...
        while True:
            try:
                ...
                for ch_db_id, ch_tele, post_id, text, media_type, media_file_id in rows:
                    tasks.append(BackgroundTasks._publish_post(bot, ch_db_id, ch_tele, post_id, text, media_type, media_file_id))
                if tasks:
                    await asyncio.gather(*tasks, return_exceptions=True)
                interval = max(60, await SettingRepository.get_publish_interval())
    ...
    @staticmethod
    async def _publish_post(bot, ch_db_id, ch_tele, post_id, text, media_type, media_file_id):
        try:
...
# =====================================================================
async def main():
    logger.info(f"🚀 Starting {CONFIG.BOT_NAME} v7.0.0-complete")
    await DB.initialize()
...
if __name__ == "__main__":
    print(f"🌿 {CONFIG.BOT_NAME} v7.0.0-complete")
    print("✅ جميع الميزات متكاملة")
    print("✅ نظام الأمان متكامل")
    print("✅ نظام الردود التلقائية مع الأزرار")
    print("✅ نظام الدفع عبر Telegram Stars")
    print("✅ نظام المسابقات والإحالات")
    print("✅ نظام التذكيرات والإشعارات")
    print("✅ دعم Webhook و Polling")
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        print("\n👋 تم الإيقاف")
