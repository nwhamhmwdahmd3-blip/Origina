...
- ✅ جميع إصلاحات المراجعة السابقة
- ✅ إضافة جميع معالجات الأمان المفقودة (sec_set_*, sec_antiflood_*, sec_night_*, sec_penalty_*)
- ✅ إصلاح admin_restore_file مع إغلاق الاتصالات وإعادة التهيئة
- ✅ توحيد معالجة set_duration و set_warn_penalty
- ✅ إضافة دالة is_admin للتحقق من صلاحيات الأدمن (مطورين + مشرفين مضافين)
- ✅ إضافة معالجات للحالات الجديدة (WAIT_ANTIFLOOD_MESSAGES, WAIT_NIGHT_START, ...)
- ✅ تقسيم handle إلى دوال فرعية لتحسين الصيانة
- ✅ تحسين معالجة الترجمة والتعامل مع lang=None
- ✅ إزالة الازدواجية وتحسين الأداء
- ✅ دعم جميع أزرار لوحة الأدمن بشكل كامل
- ✅ معالجة جميع أزرار الردود التلقائية
- ✅ معالجة جميع أزرار الجدولة
- ✅ معالجة جميع أزرار المسابقات
- ✅ معالجة جميع أزرار الإجراءات المتقدمة
- ✅ إصلاح جميع مشاكل الـ async والـ await
- ✅ إضافة توجيه أزرار قنوات الإعلانات (ad_ch_*) في بداية المعالج
"""
...
from ad_channels import handle_ad_channel_callback

# استيراد معالج قنوات الإعلانات
from ad_channels import handle_ad_channel_callback
...
async def safe_edit(query, text, reply_markup=None, parse_mode=None, bot=None):
    """تعديل الرسالة بأمان مع التعامل مع الأخطاء"""
    ...
    try:
        ...
        if "message is not modified" in error_msg:
            ...
            try:
                ...
                pass
            try:
                send_bot = bot if bot else query.bot
...
async def _is_admin(user_id: int) -> bool:
    """التحقق من صلاحيات الأدمن (مطور + مشرفين مضافين)"""
...
class CallbackHandlers:
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # ========== توجيه أزرار قنوات الإعلانات إلى معالجها المخصص ==========
        if data.startswith("ad_ch_"):
            await handle_ad_channel_callback(update, context)
            return
        ...
        # ========== معالجة الأزرار الخاصة بالعقوبات (مباشرة) ==========
        if data.startswith("set_warn_penalty:"):
            await CallbackHandlers._handle_set_warn_penalty(update, context, query, user_id, lang, data)
            return
        ...
        try:
            ...
            if base_data == CB.SETTINGS:
                await CallbackHandlers._handle_settings(update, context, query, user_id, lang)
                return

            if base_data == CB.TOGGLE_AUTO:
                await CallbackHandlers._toggle_auto_publish(update, context, query, user_id, lang)
                return

            if base_data == CB.TOGGLE_REC:
                await CallbackHandlers._toggle_auto_recycle(update, context, query, user_id, lang)
                return

            # ========== الباقات والدفع ==========
            if base_data == CB.PLANS:
                kb = await KeyboardFactory.build("plans", lang=lang)
                await safe_edit(query, "💎 اختر باقة:", reply_markup=kb, bot=context.bot)
                return

            if base_data == "gift_plans":
                await CallbackHandlers._handle_gift_plans(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.INVOICES:
                await CallbackHandlers._handle_invoices(update, context, query, user_id, lang)
                return

            # ========== الإحالات ==========
            if base_data == CB.REFERRAL:
                await CallbackHandlers._handle_referral(update, context, query, user_id, lang)
                return

            if base_data == CB.REF_CLAIM:
                await CallbackHandlers._handle_ref_claim(update, context, query, user_id, lang)
                return

            if base_data == CB.REF_LIST:
                await CallbackHandlers._handle_ref_list(update, context, query, user_id, lang)
                return

            # ========== التذكيرات ==========
            if base_data in [CB.REM_TOGGLE_SUB, CB.REM_TOGGLE_DAILY, CB.REM_TOGGLE_WEEKLY]:
                await CallbackHandlers._handle_reminder_toggle(update, context, query, user_id, lang, base_data)
                return

            if base_data == CB.REMINDER:
                await CallbackHandlers._handle_reminder_menu(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.TRANSLATION:
                await CallbackHandlers._handle_translation_menu(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.CONTEST_WINNERS:
                await CallbackHandlers._handle_contest_winners(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.CH_ADD:
                await CallbackHandlers._handle_ch_add(update, context, query, user_id, lang)
                return
            ...
            if data.startswith(CB.CH_SEL + ":"):
                await CallbackHandlers._handle_ch_sel(update, context, query, user_id, lang, data)
                return

            if data.startswith(CB.CH_DEL + ":"):
                await CallbackHandlers._handle_ch_del(update, context, query, user_id, lang, data)
                return

            if data.startswith(CB.CH_STATS + ":"):
                await CallbackHandlers._handle_ch_stats(update, context, query, user_id, lang, data)
                return

            # ========== المنشورات ==========
            if base_data == CB.POST_ADD:
                await CallbackHandlers._handle_post_add(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.POST_PUB:
                await CallbackHandlers._handle_post_pub(update, context, query, user_id, lang)
                return
            ...
            if base_data == CB.POST_REC:
                await CallbackHandlers._handle_post_rec(update, context, query, user_id, lang)
                return

            if data.startswith(CB.POST_DEL + ":"):
                await CallbackHandlers._handle_post_del(update, context, query, user_id, lang, data)
                return

            if base_data == CB.POST_CLEAR:
                await CallbackHandlers._handle_post_clear(update, context, query, user_id, lang)
                return

            if base_data == CB.PUB_ALL:
                await CallbackHandlers._handle_pub_all(update, context, query, user_id, lang)
                return

            # ========== المجموعات ==========
            if base_data == CB.GROUPS:
                await CallbackHandlers._handle_groups(update, context, query, user_id, lang)
                return

            if data.startswith("grp_del:"):
                await CallbackHandlers._handle_grp_del(update, context, query, user_id, lang, data)
                return

            if data.startswith(CB.GRP_SET + ":"):
                await CallbackHandlers._handle_grp_set(update, context, query, user_id, lang, data)
                return
            ...
            if base_data == CB.ADMIN:
                if not await _is_admin(user_id):
                    ...
                    return
                kb = await KeyboardFactory.build("admin_panel", lang=lang)
            ...
            if data.startswith("sched_open:") or data.startswith("sched_"):
                await CallbackHandlers._handle_schedule(update, context, query, user_id, lang, data)
                return

            if data.startswith("ban_") or data.startswith("act_") or data.startswith("pen_"):
                await CallbackHandlers._handle_advanced_actions(update, context, query, user_id, lang, data)
                return

            if data.startswith("contest_") or data.startswith(CB.DECLARE_WINNER_SEL + ":"):
                await CallbackHandlers._handle_contests(update, context, query, user_id, lang, data)
                return

            if data in (CB.ADMIN_IMPORT_REPLIES, CB.ADMIN_IMPORT_GITHUB):
                await CallbackHandlers._handle_import(update, context, query, user_id, lang, data)
                return

            if data.startswith("lang_"):
                await CallbackHandlers._handle_lang_set(update, context, query, user_id, lang, data)
                return
            ...
            if data in ["panel_lock", "panel_unlock", "panel_close"]:
                await CallbackHandlers._handle_panel(update, context, query, user_id, lang, data)
                return
    ...
    # =====================================================================
    # دوال معالجة الأزرار المساعدة
    # =====================================================================

    @staticmethod
    async def _handle_set_warn_penalty(update, context, query, user_id, lang, data):
        """معالج set_warn_penalty: تعيين عقوبة التحذير"""
        try:
    ...
    async def _handle_set_duration(update, context, query, user_id, lang, data):
        """معالج set_duration: تعيين مدة العقوبة"""
        try:
            parts = data.split(":")
            if len(parts) >= 4:
            ...
            logger.error(f"خطأ في set_duration: {e}", exc_info=True)
            await _safe_answer(query, "❌ بيانات غير صالحة", show_alert=True)
    ...
    @staticmethod
    async def _handle_settings(update, context, query, user_id, lang):
        ...
        kb = await KeyboardFactory.build("settings", lang=lang)
        await safe_edit(query, f"⚙️ الإعدادات\n\n{auto_label}: {auto}\n{recycle_label}: {rec}", reply_markup=kb, bot=context.bot)

    @staticmethod
    async def _toggle_auto_publish(update, context, query, user_id, lang):
        cur = await DB.get_auto_publish_status(user_id)
        await DB.set_auto_publish(user_id, not cur)
        await CallbackHandlers._handle_settings(update, context, query, user_id, lang)

    @staticmethod
    async def _toggle_auto_recycle(update, context, query, user_id, lang):
        cur = await DB.get_auto_recycle_status(user_id)
        await DB.set_auto_recycle(user_id, not cur)
        await CallbackHandlers._handle_settings(update, context, query, user_id, lang)

    # =====================================================================
    # دوال معالجة الباقات والدفع
    # =====================================================================

    @staticmethod
    async def _handle_gift_plans(update, context, query, user_id, lang):
    ...
    @staticmethod
    async def _handle_invoices(update, context, query, user_id, lang):
    ...
    # =====================================================================
    # دوال معالجة الإحالات
    # =====================================================================

    @staticmethod
    async def _handle_referral(update, context, query, user_id, lang):
    ...
    @staticmethod
    async def _handle_ref_claim(update, context, query, user_id, lang):
        days = await DB.claim_referral_reward(user_id)
        text = f"✅ تم صرف {days} يوم!" if days > 0 else "📭 لا توجد مكافآت"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=CB.REFERRAL)]])
        await safe_edit(query, text, reply_markup=kb, bot=context.bot)

    @staticmethod
    async def _handle_ref_list(update, context, query, user_id, lang):
    ...
    # =====================================================================
    # دوال معالجة التذكيرات
    # =====================================================================

    @staticmethod
    async def _handle_reminder_toggle(update, context, query, user_id, lang, base_data):
    ...
    @staticmethod
    async def _handle_reminder_menu(update, context, query, user_id, lang):
        settings = await DB.get_reminder_settings(user_id) or {}
        text = (
            f"⏰ التذكيرات\n\n"
        ...
        kb = await KeyboardFactory.build("reminder", lang=lang)
        await safe_edit(query, text, reply_markup=kb, bot=context.bot)
    ...
    @staticmethod
    async def _handle_translation_menu(update, context, query, user_id, lang):
        kb = InlineKeyboardMarkup([
    ...
    @staticmethod
    async def _handle_lang_set(update, context, query, user_id, lang, data):
    ...
    # =====================================================================
    # دوال معالجة المسابقات (العامة)
    # =====================================================================

    @staticmethod
    async def _handle_contest_winners(update, context, query, user_id, lang):
    ...
    @staticmethod
    async def _handle_ch_add(update, context, query, user_id, lang):
    ...
    async def _show_channel_list(update, context, query, user_id, lang=None):
        ...
        if not channels:
            ch_add_text = await KeyboardFactory.get_text("ch_add", lang)
            back_text = await KeyboardFactory.get_text("back", lang)
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton(ch_add_text, callback_data=CB.CH_ADD)],
        ...
        ch_add_text = await KeyboardFactory.get_text("ch_add", lang)
        back_text = await KeyboardFactory.get_text("back", lang)
        kb.append([InlineKeyboardButton(ch_add_text, callback_data=CB.CH_ADD)])
    ...
    @staticmethod
    async def _handle_ch_sel(update, context, query, user_id, lang, data):
        try:
    ...
    @staticmethod
    async def _handle_ch_del(update, context, query, user_id, lang, data):
        try:
            ch_id = int(data.split(":")[-1])
        except (ValueError, IndexError):
            await _safe_answer(query, "❌ بيانات غير صالحة", show_alert=True)
    ...
    @staticmethod
    async def _handle_ch_stats(update, context, query, user_id, lang, data):
        try:
            ch_id = int(data.split(":")[-1])
        except (ValueError, IndexError):
            await _safe_answer(query, "❌ بيانات غير صالحة", show_alert=True)
        ...
        text = f"📊 إحصائيات\n\n📝 {stats['total']}\n✅ {stats['published']}\n⏳ {stats['unpublished']}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=CB.CH_LIST)]])
        await safe_edit(query, text, reply_markup=kb, bot=context.bot)

    # =====================================================================
    # دوال معالجة المنشورات
    # =====================================================================

    @staticmethod
    async def _handle_post_add(update, context, query, user_id, lang):
        if not await DB.has_active_subscription(user_id) and user_id != CONFIG.PRIMARY_OWNER_ID:
        ...
        await safe_edit(query, "📥 أرسل المنشورات:", reply_markup=kb, bot=context.bot)

    @staticmethod
    async def _handle_post_pub(update, context, query, user_id, lang):
        active = await DB.get_active_channel(user_id)
        if not active:
        ...
        if ch_info:
            ...
            await _safe_answer(query, "❌ تعذر العثور على بيانات القناة", show_alert=True)
    ...
    async def _show_post_list(update, context, query, user_id, lang=None):
        ...
        if nav:
            kb.append(nav)
        rec_text = await KeyboardFactory.get_text("post_rec", lang) or "🔄 إعادة تدوير"
        clear_text = await KeyboardFactory.get_text("post_clear", lang) or "🧹 مسح الكل"
        back_text = await KeyboardFactory.get_text("back", lang)
        kb.append([InlineKeyboardButton(rec_text, callback_data=CB.POST_REC)])
        kb.append([InlineKeyboardButton(clear_text, callback_data=CB.POST_CLEAR)])
        kb.append([InlineKeyboardButton(back_text, callback_data=CB.BACK)])
    ...
    @staticmethod
    async def _handle_post_rec(update, context, query, user_id, lang):
        active = await DB.get_active_channel(user_id)
        if active:
    ...
    @staticmethod
    async def _handle_post_del(update, context, query, user_id, lang, data):
        try:
    ...
    @staticmethod
    async def _handle_post_clear(update, context, query, user_id, lang):
        active = await DB.get_active_channel(user_id)
        if active:
    ...
    @staticmethod
    async def _handle_pub_all(update, context, query, user_id, lang):
        channels = await DB.get_user_channels(user_id)
        if not channels:
    ...
    @staticmethod
    async def _handle_groups(update, context, query, user_id, lang):
    ...
    @staticmethod
    async def _handle_grp_del(update, context, query, user_id, lang, data):
        try:
    ...
    @staticmethod
    async def _handle_grp_set(update, context, query, user_id, lang, data):
        try:
            chat_id = int(data.split(":")[-1])
        except (ValueError, IndexError):
            await _safe_answer(query, "❌ بيانات غير صالحة", show_alert=True)
    ...
    async def _handle_security(update, context, query, user_id, lang=None):
        ...
        try:
            ...
            # ===== عقوبات الحذف =====
            if action == "penalty":
                await CallbackHandlers._show_penalty_types(update, context, query, chat_id, lang, for_penalty='delete')
                return
            ...
            if action == "antiflood_penalty":
                ...
                await CallbackHandlers._show_penalty_types(update, context, query, chat_id, lang, for_penalty='antiflood')
                return

            if action == "antiflood_duration":
                await CallbackHandlers._show_penalty_durations(update, context, query, chat_id, lang, penalty_type='antiflood')
                return
            ...
            if action == "night_action":
                ...
                await CallbackHandlers._show_penalty_types(update, context, query, chat_id, lang, for_penalty='night')
                return

            if action == "night_duration":
                await CallbackHandlers._show_penalty_durations(update, context, query, chat_id, lang, penalty_type='night')
                return
            ...
            # ===== عقوبات الحذف (sec_penalty_*) =====
            if action.startswith("penalty_"):
                ptype = action.split("_")[-1]
                if ptype in DB.VALID_PENALTY_TYPES:
                    await DB.update_security_settings(chat_id, delete_penalty=ptype)
                    await _safe_answer(query, f"✅ تم تعيين عقوبة الحذف: {ptype}")
                    settings = await DB.get_security_settings(chat_id)
                    kb = await KeyboardFactory.build("security", chat_id=chat_id, lang=lang)
                    await safe_edit(query, KeyboardFactory._format_security_text(settings),
                                    reply_markup=kb,
    ...
    # =====================================================================
    # دوال مساعدة للأمان (عرض القوائم)
    # =====================================================================
    ...
    @staticmethod
    async def _show_penalty_types(update, context, query, chat_id, lang, for_penalty='delete'):
        penalty_types = [
            ("🔇 كتم", "mute"),
            ("🚫 حظر", "ban"),
            ("👢 طرد", "kick"),
        ...
        for label, ptype in penalty_types:
            if for_penalty == 'delete':
                cb = f"sec_penalty_{ptype}:{chat_id}"
            elif for_penalty == 'antiflood':
                cb = f"sec_set_antiflood_penalty:{chat_id}:{ptype}"
            elif for_penalty == 'night':
                cb = f"sec_set_night_action:{chat_id}:{ptype}"
            else:
                cb = f"sec_penalty_{ptype}:{chat_id}"
    ...
    async def _show_auto_reply_menu(update, context, query, chat_id, lang):
        kb = await KeyboardFactory.build("auto_reply", chat_id=chat_id, lang=lang)
    ...
    async def _show_advanced_actions(update, context, query, chat_id, lang):
        kb = await KeyboardFactory.build("advanced_actions", chat_id=chat_id, lang=lang)
    ...
    async def _show_admin_logs(update, context, query, chat_id, lang):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=f"grp_set:{chat_id}")]])
        await safe_edit(query, text, reply_markup=kb, bot=context.bot)

    # =====================================================================
    # دوال معالجة لوحة الأدمن (admin_*)
    # =====================================================================
    ...
    async def _handle_admin(update, context, query, user_id, lang=None):
        ...
        try:
            ...
            if data == CB.ADMIN_RESTORE_SEL:
                await CallbackHandlers._show_restore_backups(update, context, query, user_id)
                return

            if data.startswith("admin_restore_file:"):
                await CallbackHandlers._handle_restore_file(update, context, query, user_id, data)
                return
    ...
    @staticmethod
    async def _handle_restore_file(update, context, query, user_id, data):
        ...
        try:
            ...
            await DB._close_all_connections()
            shutil.copy2(backup_file, PATHS.DB)
            await DB.initialize()
            await safe_edit(query, "✅ تمت الاستعادة بنجاح! يُفضل إعادة تشغيل البوت.", bot=context.bot)
        except Exception as e:
    ...
    async def _handle_auto_reply(update, context, query, user_id, lang=None):
        ...
        try:
            if action == "toggle":
                ...
                back_text = await KeyboardFactory.get_text("back", lang)
                kb = InlineKeyboardMarkup([[InlineKeyboardButton(back_text, callback_data=f"auto_reply_menu:{chat_id}")]])
                await safe_edit(query, text, reply_markup=kb, bot=context.bot)
                return
    ...
    @staticmethod
    async def _handle_schedule(update, context, query, user_id, lang, data):
        parts = data.split(":")
        if len(parts) < 2:
            return
        ...
        if action == "open":
            await CallbackHandlers._show_schedule_menu(update, context, query, ch_id, user_id, lang)
        elif action == "min":
    ...
    @staticmethod
    async def _show_schedule_menu(update, context, query, ch_id, user_id, lang):
        kb = await KeyboardFactory.build("channel_settings", chat_id=ch_id, lang=lang)
        await safe_edit(query, "📅 جدولة القناة", reply_markup=kb, bot=context.bot)

    # =====================================================================
    # دوال معالجة الإجراءات المتقدمة والعقوبات
    # =====================================================================

    @staticmethod
    async def _handle_advanced_actions(update, context, query, user_id, lang, data):
        parts = data.split(":")
        if len(parts) < 2:
            return
        ...
        if chat_id != -1 and not await is_authorized_in_group(context.bot, chat_id, user_id):
            await _safe_answer(query, "❌ لا صلاحية", show_alert=True)
            return
        if chat_id == -1 and not await _is_admin(user_id):
            await _safe_answer(query, "❌ غير مصرح", show_alert=True)
            return
    ...
    # =====================================================================
    # دوال معالجة المسابقات (المتقدمة)
    # =====================================================================

    @staticmethod
    async def _handle_contests(update, context, query, user_id, lang, data):
        try:
    ...
    @staticmethod
    async def _handle_import(update, context, query, user_id, lang, data):
        if not await _is_admin(user_id):
            await _safe_answer(query, "❌ غير مصرح", show_alert=True)
            return
        if data == CB.ADMIN_IMPORT_REPLIES:
    ...
    # =====================================================================
    # دوال معالجة اللوحة الخاصة (panel)
    # =====================================================================

    @staticmethod
    async def _handle_panel(update, context, query, user_id, lang, data):
        chat_id = update.effective_chat.id
        if not await is_authorized_in_group(context.bot, chat_id, user_id):
            await _safe_answer(query, "❌ لا صلاحية", show_alert=True)
            return
