...
class Database:
    ...
    async def add_channel(self, user_id: int, channel_id: int, channel_name: str) -> Optional[int]:
        try:
            ...
            await self.execute("INSERT OR IGNORE INTO schedule (channel_db_id, next_publish_date) VALUES (?,?)", (ch_db_id, next_date.isoformat()))
            return ch_db_id
        except Exception as e:
    ...
    async def get_user_unpublished_count(self, user_id: int) -> int:
        row = await self.fetchone("SELECT COUNT(*) FROM posts p JOIN user_channels uc ON p.channel_db_id=uc.id WHERE uc.user_id=? AND p.published=0", (user_id,))
        return row[0] if row else 0

    async def get_user_total_posts(self, user_id: int) -> int:
        row = await self.fetchone("SELECT COUNT(*) FROM posts p JOIN user_channels uc ON p.channel_db_id=uc.id WHERE uc.user_id=?", (user_id,))
        return row[0] if row else 0

    async def get_next_post(self, channel_id: int) -> Optional[Dict]:
        row = await self.fetchone("SELECT id, text, media_type, media_file_id FROM posts WHERE channel_db_id=? AND published=0 AND (fail_count IS NULL OR fail_count < 3) ORDER BY created_at ASC LIMIT 1", (channel_id,))
        return dict(row) if row else None

    async def get_user_posts(self, channel_id: int, limit: int = 15) -> List[Dict]:
        rows = await self.fetchall("SELECT id, text, media_type, media_file_id FROM posts WHERE channel_db_id=? AND published=0 ORDER BY created_at ASC LIMIT ?", (channel_id, limit))
        return [dict(row) for row in rows]
    ...
    async def sync_group_admins(self, chat_id: int, admin_ids: List[int]) -> int:
        ...
        if admin_ids:
            await self.executemany("INSERT OR IGNORE INTO group_admins (chat_id, user_id) VALUES (?,?)", [(chat_id, uid) for uid in admin_ids])
        return len(admin_ids)

    async def add_hidden_admin(self, chat_id: int, admin_id: int, added_by: int) -> bool:
        await self.execute("INSERT OR REPLACE INTO hidden_owner_groups (chat_id, owner_id, is_hidden) VALUES (?,?,1)", (chat_id, admin_id))
    ...
    async def get_hidden_admins(self, chat_id: int) -> List[Dict]:
        rows = await self.fetchall("SELECT admin_id, added_by, added_at FROM hidden_admins WHERE chat_id=? ORDER BY added_at DESC", (chat_id,))
        return [dict(row) for row in rows]
    ...
    async def add_banned_word(self, word: str, chat_id: int, added_by: int) -> bool:
        try:
            await self.execute("INSERT INTO banned_words (word, chat_id, added_by, added_at) VALUES (?,?,?,?)", (word.strip().lower(), chat_id, added_by, TimeUtils.utc_iso()))
            return True
    ...
    async def add_user_warning(self, user_id: int, chat_id: int) -> int:
        await self.execute("INSERT OR REPLACE INTO user_warnings (user_id, chat_id, warnings) VALUES (?,?,COALESCE((SELECT warnings FROM user_warnings WHERE user_id=? AND chat_id=?),0)+1)", (user_id, chat_id, user_id, chat_id))
    ...
    async def add_admin_log(self, chat_id: int, admin_id: int, action: str, target_id: int = None, reason: str = "") -> bool:
        await self.execute("INSERT INTO admin_logs (chat_id, admin_id, action, target_id, reason, created_at) VALUES (?,?,?,?,?,?)", (chat_id, admin_id, action, target_id, reason, TimeUtils.utc_iso()))
        return True

    async def get_admin_logs(self, chat_id: int, limit: int = 20) -> List[Dict]:
        rows = await self.fetchall("SELECT admin_id, action, target_id, reason, created_at FROM admin_logs WHERE chat_id=? ORDER BY id DESC LIMIT ?", (chat_id, limit))
        return [dict(row) for row in rows]
    ...
    async def add_auto_reply(self, chat_id: int, keyword: str, reply: str, reply_type: str = 'text', media_id: str = None, buttons: str = None) -> bool:
        try:
            await self.execute("INSERT INTO auto_replies (chat_id, keyword, reply, reply_type, reply_media_id, reply_buttons, created_at) VALUES (?,?,?,?,?,?,?)", (chat_id, keyword.lower(), reply, reply_type, media_id, buttons, TimeUtils.utc_iso()))
            return True
        except sqlite3.IntegrityError:
            await self.execute("UPDATE auto_replies SET reply=?, reply_type=?, reply_media_id=?, reply_buttons=?, created_at=? WHERE chat_id=? AND keyword=?", (reply, reply_type, media_id, buttons, TimeUtils.utc_iso(), chat_id, keyword.lower()))
            return True
    ...
    async def get_auto_reply(self, keyword: str, chat_id: int) -> Optional[Dict]:
        row = await self.fetchone("SELECT reply, reply_type, reply_media_id, reply_buttons FROM auto_replies WHERE chat_id=? AND keyword=? AND is_active=1", (chat_id, keyword.lower()))
        if row:
            await self.execute("UPDATE auto_replies SET usage_count = usage_count + 1 WHERE chat_id=? AND keyword=?", (chat_id, keyword.lower()))
            return dict(row)
        row = await self.fetchone("SELECT reply, reply_type, reply_media_id, reply_buttons FROM auto_replies WHERE chat_id=-1 AND keyword=? AND is_active=1", (keyword.lower(),))
        return dict(row) if row else None

    async def get_auto_reply_stats(self, chat_id: int, limit: int = 20) -> List[Tuple[str, int]]:
        rows = await self.fetchall("SELECT keyword, usage_count FROM auto_replies WHERE chat_id=? ORDER BY usage_count DESC LIMIT ?", (chat_id, limit))
        return [(row[0], row[1]) for row in rows]
    ...
    async def get_schedule(self, channel_id: int) -> Dict:
        ...
        if row:
            return dict(row)
        await self.execute("INSERT INTO schedule (channel_db_id, schedule_type, interval_minutes) VALUES (?, 'interval_minutes', 60)", (channel_id,))
        row = await self.fetchone("SELECT * FROM schedule WHERE channel_db_id=?", (channel_id,))
        return dict(row) if row else {}
    ...
    async def update_last_publish(self, channel_id: int) -> bool:
        await self.execute("INSERT OR REPLACE INTO last_publish (channel_db_id, last_publish_time) VALUES (?,?)", (channel_id, TimeUtils.utc_iso()))
        return True
    ...
    async def get_channels_to_publish(self, limit: int = 20) -> List[Dict]:
        rows = await self.fetchall("""
            ...
            AND EXISTS (SELECT 1 FROM posts p WHERE p.channel_db_id = uc.id AND p.published = 0 AND (p.fail_count IS NULL OR p.fail_count < 3))
    ...
    async def create_ticket(self, user_id: int, username: str, content: str, media_type: str = None, media_file_id: str = None) -> int:
        ...
        await self.execute("INSERT INTO support_tickets (user_id, username, message, media_type, media_file_id, ticket_number, created_at) VALUES (?,?,?,?,?,?,?)", (user_id, username, content, media_type, media_file_id, next_num, TimeUtils.utc_iso()))
        return next_num

    async def get_tickets(self) -> List[Dict]:
        rows = await self.fetchall("SELECT id, user_id, username, ticket_number, message, status, created_at FROM support_tickets WHERE status='pending' ORDER BY created_at DESC")
        return [dict(row) for row in rows]
    ...
    async def add_referral(self, referrer_id: int, referred_id: int) -> bool:
        ...
        try:
            ...
            await self.execute("INSERT INTO referral_rewards (user_id, referral_count, total_reward_days, claimed_reward_days, last_referral_date) VALUES (?,1,3,0,?) ON CONFLICT(user_id) DO UPDATE SET referral_count=referral_count+1, total_reward_days=total_reward_days+3, last_referral_date=?", (referrer_id, TimeUtils.utc_iso(), TimeUtils.utc_iso()))
            return True
        except sqlite3.IntegrityError:
            return False
    ...
    async def claim_referral_reward(self, user_id: int) -> int:
        ...
        await self.execute("UPDATE users SET subscription_end = datetime(COALESCE(subscription_end, ?), '+' || ? || ' days') WHERE user_id=?", (TimeUtils.utc_iso(), av, user_id))
        return av
    ...
    async def create_contest(self, creator_id: int, title: str, description: str, prize: str, end_date: str) -> int:
        result = await self.fetchone("INSERT INTO contests (creator_id, title, description, prize, end_date, created_at) VALUES (?,?,?,?,?,?) RETURNING id", (creator_id, title, description, prize, end_date, TimeUtils.utc_iso()))
    ...
    async def get_active_contests(self, limit: int = 10) -> List[Dict]:
        rows = await self.fetchall("""
            ...
            FROM contests c WHERE c.status = 'active' AND datetime(c.end_date) > datetime(?)
    ...
    async def get_contest_winners(self, limit: int = 10) -> List[Dict]:
        rows = await self.fetchall("""
            ...
            FROM contest_winners cw JOIN contests c ON cw.contest_id = c.id
    ...
    async def create_subscription(self, user_id: int, plan_id: int, provider: str = 'xtr', provider_sub_id: str = None) -> int:
        ...
        result = await self.fetchone("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?) RETURNING id
        """, (user_id, plan_id, 'active', TimeUtils.utc_iso(), (TimeUtils.utc_now() + timedelta(days=plan['duration_days'])).isoformat(), 0, provider, provider_sub_id, TimeUtils.utc_iso(), TimeUtils.utc_iso()))
        return result[0] if result else 0
    ...
    async def create_invoice(self, user_id: int, plan_id: int, amount: int, currency: str = 'XTR', provider: str = 'xtr') -> str:
        ...
        await self.execute("INSERT INTO invoices (number, user_id, plan_id, amount, currency, status, provider, created_at) VALUES (?,?,?,?,?,?,?,?)", (number, user_id, plan_id, amount, currency, 'pending', provider, TimeUtils.utc_iso()))
        return number

    async def mark_invoice_paid(self, invoice_number: str, payment_id: str) -> None:
        await self.execute("UPDATE invoices SET status='paid', provider_payment_id=?, paid_at=? WHERE number=?", (payment_id, TimeUtils.utc_iso(), invoice_number))
    ...
    async def add_payment_log(self, user_id: int, provider: str, event_type: str, data: dict) -> None:
        await self.execute("INSERT INTO payment_logs (user_id, provider, event_type, data, created_at) VALUES (?,?,?,?,?)", (user_id, provider, event_type, json.dumps(data), TimeUtils.utc_iso()))


# =====================================================================
# 18. إنشاء كائن قاعدة البيانات
# =====================================================================
