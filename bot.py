...
"""
🌿 Relax Manager - النسخة النهائية الكاملة (ملف واحد)
================================================================================
بوت تلغرام متكامل لإدارة القنوات والمجموعات مع نظام دفع عبر Telegram Stars

الميزات الرئيسية:
- إدارة القنوات والمنشورات مع جدولة نشر مرنة
- نظام أمان شامل للمجموعات
- اشتراكات عبر Telegram Stars (XTR)
- إحالات ومكافآت
- مسابقات وتذاكر دعم
- ردود تلقائية
- لوحة تحكم للمشرفين
...
@RelaxMgr | الإصدار 5.0.0-final
================================================================================
"""

# =====================================================================
# 1. تثبيت الحزم والاستيرادات
# =====================================================================
import asyncio
import sys
import os
...
async def initialize(self) -> None:
    ...
    async def _create_tables(self) -> None:
        tables = [
            ...
            """,
            "CREATE INDEX IF NOT EXISTS idx_bw_word ON banned_words(word)",
            """
            ...
            """,
            # ===== جداول الاشتراكات والدفع (Telegram Stars) =====
            """
...
# =====================================================================
# 7. مستودعات البيانات (Repositories)
# =====================================================================
class UserRepository:
    ...
    async def has_active_subscription(user_id: int) -> bool:
        ...
        if row and row[0]:
            try:
                return datetime.fromisoformat(row[0]) > TimeUtils.utc_now()
            except:
                pass
        return False
    ...
    async def activate_subscription(user_id: int, days: int) -> None:
        ...
        if row and row[0]:
            try:
                ...
                new_end = (current if current > TimeUtils.utc_now() else TimeUtils.utc_now()) + timedelta(days=days)
            except:
...
class ChannelRepository:
    ...
    async def add(user_id: int, channel_id: str, channel_name: str) -> Optional[int]:
        ...
        if ch_db_id:
            interval = await SettingRepository.get_publish_interval()
    ...
    async def get_active(user_id: int) -> Optional[int]:
        ...
        if row and row[0]:
            ...
            if banned and banned[0] == 0:
                return row[0]
        row2 = await DB.fetchone("SELECT id FROM user_channels WHERE user_id=? AND banned=0 ORDER BY id LIMIT 1",
                                 (user_id,))
    ...
    async def get_stats(channel_db_id: int) -> Dict[str, int]:
        ...
        published = (await DB.fetchone("SELECT COUNT(*) FROM posts WHERE channel_db_id=? AND published=1",
                                       (channel_db_id,)))[0]
...
class PostRepository:
    ...
    async def delete_single(post_id: int, user_id: int, channel_db_id: int) -> bool:
        row = await DB.fetchone("SELECT 1 FROM user_channels WHERE id=? AND user_id=? AND banned=0",
...
class ScheduleRepository:
    ...
    async def update_next(channel_db_id: int) -> None:
        ...
        last_time = datetime.fromisoformat(row[0]) if row and row[0] else TimeUtils.utc_now()
    ...
    async def register(chat_id: int, chat_name: str, added_by: int, username: str = None) -> bool:
        ...
        if row:
            await DB.execute("UPDATE bot_groups SET chat_name=?, username=?, added_by=?, updated_at=? WHERE chat_id=?",
                             (chat_name[:255], username[:100] if username else None, added_by, TimeUtils.utc_iso(),
                              chat_id))
            return not row[1]
        await DB.execute(
...
# =====================================================================
# 9. الردود التلقائية والتذاكر والإحالات
# =====================================================================
class AutoReplyRepository:
    ...
    async def get_settings(cls, chat_id: int) -> dict:
        ...
        row = await DB.fetchone("SELECT enabled, only_admins, ignore_bots FROM auto_reply_settings WHERE chat_id=?",
                                (chat_id,))
        if row:
    ...
    async def get_reply(cls, keyword: str, chat_id: int = 0) -> Optional[dict]:
        row = await DB.fetchone(
            ...
            (chat_id, keyword))
        if row:
            await DB.execute("UPDATE auto_replies SET usage_count = usage_count + 1 WHERE chat_id=? AND keyword=?",
                             (chat_id, keyword))
            return {'reply': row[0], 'type': row[1], 'media_id': row[2], 'buttons': json.loads(row[3]) if row[3] else None}
        return None
...
class TicketRepository:
    @staticmethod
    async def save(user_id: int, username: str, message: str, ticket_num: int) -> None:
        await DB.execute("INSERT INTO support_tickets (user_id, username, message, ticket_number, created_at) VALUES (?,?,?,?,?)",
                         (user_id, username, message, ticket_num, TimeUtils.utc_iso()))
    ...
    async def get_all() -> List[dict]:
        rows = await DB.fetchall(
            "SELECT id, user_id, username, message, ticket_number, status, created_at FROM support_tickets ORDER BY created_at DESC LIMIT 20")
        return [dict(row) for row in rows]
...
class ReferralRepository:
    ...
    async def add(referrer_id: int, referred_id: int) -> bool:
        ...
        count = (await DB.fetchone("SELECT COUNT(*) FROM referrals WHERE referrer_id=? AND date(created_at)=?",
                                   (referrer_id, today)))[0]
    ...
    async def get_stats(user_id: int) -> dict:
        ...
        row = await DB.fetchone(
            ...
            (user_id,))
        if row:
            return {'total': total, 'count': row[0], 'total_days': row[1], 'claimed': row[2],
    ...
    async def claim(user_id: int) -> int:
        ...
        if row and row[0]:
            try:
                ...
                if ed > TimeUtils.utc_now():
                    cd = (ed - TimeUtils.utc_now()).days
            except:
                pass
        new_end = (TimeUtils.utc_now() + timedelta(days=cd + av)).isoformat()
        await DB.execute("UPDATE users SET subscription_end=? WHERE user_id=?", (new_end, user_id))
        await DB.execute("UPDATE referral_rewards SET claimed_reward_days=claimed_reward_days+? WHERE user_id=?",
                         (av, user_id))
        return av

    @staticmethod
    async def get_list(user_id: int) -> List[int]:
        rows = await DB.fetchall("SELECT referred_id FROM referrals WHERE referrer_id=? ORDER BY created_at DESC",
                                 (user_id,))
        return [row[0] for row in rows]


# =====================================================================
# 10. نظام الدفع عبر Telegram Stars
# =====================================================================
...
class PaymentService:
    """خدمة الدفع عبر Telegram Stars"""

    @staticmethod
    async def create_payment(user_id: int, plan_id: int) -> Tuple[Optional[str], Optional[str]]:
        """إنشاء فاتورة وإرجاع رقم الفاتورة وبيانات الدفع"""
        plan = await PlanRepository.get_by_id(plan_id)
        if not plan:
            return None, None
        ...
        # إنشاء بيانات الدفع لـ Telegram Stars
        payment_data = {
            ...
            'payload': f"sub_{plan_id}_{invoice_number}"
        }
    ...
    async def handle_successful_payment(user_id: int, payload: str) -> bool:
        """معالجة الدفع الناجح وتفعيل الاشتراك"""
        try:
...
# =====================================================================
# 11. المسابقات والتذكيرات
# =====================================================================
class ContestRepository:
    @staticmethod
    async def create(creator_id: int, title: str, description: str, prize: str, end_date: datetime,
                     contest_type: str = 'raffle') -> Optional[int]:
        result = await DB.fetchone(
...
class ReminderRepository:
    ...
    async def get_settings(user_id: int) -> dict:
        row = await DB.fetchone(
            ...
            (user_id,))
        if row:
            return {'sub': row[0] == 1, 'daily': row[1] == 1, 'weekly': row[2] == 1, 'days': row[3] or 3,
                    'lang': row[4] or 'ar'}
...
class SentimentAnalyzer:
    def __init__(self):
        self.positive = {"جميل", "رائع", "ممتاز", "حلو", "شكرا", "شكراً", "تسلم", "فرح", "سعيد", "مبسوط", "الحمد", "تفاؤل",
                         "أمل", "نجاح", "مبدع", "خير", "بركة", "نعمة"}
        self.negative = {"زعل", "حزين", "متعب", "محبط", "غضب", "غاضب", "مزعج", "سيء", "سخيف", "غبي", "ممل", "كره", "موت", "ألم",
                         "جرح", "نكد", "فشل", "خسر", "ظلم", "حرب", "شر", "لعنة"}
...
async def save_sentiment(user_id: int, chat_id: int, text: str, sentiment: str, score: float) -> None:
    encrypted = ENCRYPT.encrypt_text(text)
    await DB.execute(
        "INSERT INTO sentiment_history (user_id, chat_id, text_encrypted, sentiment, score, created_at) VALUES (?,?,?,?,?,?)",
        (user_id, chat_id, encrypted, sentiment, score, TimeUtils.utc_iso()))


# =====================================================================
# 13. الصلاحيات والمصادقة
# =====================================================================
...
class KeyboardFactory:
    @staticmethod
    def security(chat_id: int) -> InlineKeyboardMarkup:
    ...
    def plans(lang: str = 'ar') -> InlineKeyboardMarkup:
        buttons = [
            [InlineKeyboardButton("💎 1 يوم - 5 نجوم", callback_data=f"{CB.BUY_SUB}1"),
             InlineKeyboardButton("💎 7 أيام - 25 نجوم", callback_data=f"{CB.BUY_SUB}7")],
            [InlineKeyboardButton("💎 30 يوم - 75 نجوم", callback_data=f"{CB.BUY_SUB}30"),
             InlineKeyboardButton("💎 90 يوم - 200 نجوم", callback_data=f"{CB.BUY_SUB}90")],
            [InlineKeyboardButton("🔙 رجوع", callback_data=CB.BACK)]
        ]
        return InlineKeyboardMarkup(buttons)

    @staticmethod
    def banned_words(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
    ...
    @staticmethod
    def advanced_actions(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
    ...
    @staticmethod
    def mute_duration(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
    ...
    @staticmethod
    def penalty(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
    ...
    @staticmethod
    def auto_reply_settings(chat_id: int, settings: dict) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(f"👥 المستخدمون: {at}", callback_data=f"{CB.AUTO_REPLY_ADMINS}{chat_id}")],
    ...
    @staticmethod
    def auto_reply_manage(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
    ...
    async def main_menu(user_id: int) -> tuple:
        ...
        title = get_text(lang, 'main_menu',
                         ...
                         auto=auto_text)
        kb = []
        kb.append([InlineKeyboardButton("👥 مجموعاتي", callback_data=CB.GROUPS),
                   InlineKeyboardButton("➕ إضافة قناة", callback_data=CB.CH_ADD)])
        kb.append([InlineKeyboardButton("📡 قنواتي", callback_data=CB.CH_LIST),
                   InlineKeyboardButton("⚙️ الإعدادات", callback_data=CB.SETTINGS)])
        if channels:
            kb.append([InlineKeyboardButton("📥 إضافة منشورات", callback_data=CB.POST_ADD),
                       InlineKeyboardButton("📤 نشر واحد", callback_data=CB.POST_PUB)])
            kb.append([InlineKeyboardButton("📋 منشوراتي", callback_data=CB.POST_LIST),
                       InlineKeyboardButton("♻️ إعادة تدوير", callback_data=CB.POST_REC)])
            kb.append([InlineKeyboardButton(f"📊 إحصائيات ({cnt})", callback_data=CB.STATS_PEND),
                       InlineKeyboardButton("📈 كاملة", callback_data=CB.STATS_FULL)])
            if active:
            ...
            kb.append([InlineKeyboardButton("📤 نشر الكل", callback_data=CB.PUB_ALL)])
        kb.append([InlineKeyboardButton("❓ مساعدة", callback_data=CB.HELP),
                   InlineKeyboardButton("🎁 تجربة", callback_data=CB.TRIAL)])
        kb.append([InlineKeyboardButton("💎 اشتراك", callback_data=CB.SUBSCRIBE),
                   InlineKeyboardButton("👨‍💻 المطور", callback_data=CB.DEVELOPER)])
        kb.append([InlineKeyboardButton("🌐 اللغة", callback_data="language"),
                   InlineKeyboardButton("📞 دعم", callback_data=CB.SUPPORT)])
        kb.append([InlineKeyboardButton("🔗 إحالات", callback_data=CB.REFERRAL),
                   InlineKeyboardButton("⏰ تذكيرات", callback_data=CB.REMINDER)])
        kb.append([InlineKeyboardButton("🌐 ترجمة", callback_data=CB.TRANSLATION),
                   InlineKeyboardButton("🏆 مسابقات", callback_data=CB.CONTESTS)])
        kb.append([InlineKeyboardButton("➕ أضف لمجموعة", url=f"https://t.me/{CONFIG.BOT_USERNAME}?startgroup")])
        if await BotAdminRepository.is_admin(user_id):
            kb.append([InlineKeyboardButton("👑 لوحة الأدمن", callback_data=CB.ADMIN)])
        return InlineKeyboardMarkup(kb), title


# =====================================================================
# 16. إدارة حالات المستخدم
# =====================================================================
...
class CommandHandlers:
        ...
        if args and args[0].startswith('ref_'):
            ...
            if referrer and referrer != user_id:
                if await ReferralRepository.add(referrer, user_id):
                    await ReferralRepository.auto_reward(referrer)
                    await safe_send(update.effective_chat.bot, referrer, f"🎁 تمت إحالة `{user_id}`")
    ...
    async def support(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await safe_send(context.bot, user_id, "📞 الدعم الفني", reply_markup=kb)
    ...
    async def security(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        text = KeyboardFactory._build_security_text(settings)
        await safe_send(context.bot, user_id, text, reply_markup=KeyboardFactory.security(chat_id), parse_mode="HTML")
    ...
    async def panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 إغلاق", callback_data=CB.PANEL_CLOSE)]
        ])
    ...
    async def contests(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not contests:
            await safe_send(context.bot, user_id, "🏆 لا توجد مسابقات")
            return
        text = "🏆 **المسابقات**\n"
        kb = []
        for c in contests:
...
class CallbackHandlers:
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data.startswith(CB.BUY_SUB):
            ...
            if not plan:
                await query.answer("❌ الباقة غير متوفرة", show_alert=True)
                return

            # إنشاء الفاتورة وبيانات الدفع
            invoice_number, payment_data = await PaymentService.create_payment(user_id, plan['id'])
            if not invoice_number:
                await query.answer("❌ فشل إنشاء الدفع", show_alert=True)
                return
        ...
        if data == CB.INVOICES:
            ...
            text = "🧾 **فواتيري**\n"
            for inv in invoices:
                text += f"• #{inv['number']} - {inv['amount']} {inv['currency']} - {inv['status']}\n"
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=CB.BACK)]])
        ...
        if data == CB.CH_ADD:
            ...
            return
        if data == CB.CH_LIST:
            await CallbackHandlers._my_channels(query, user_id)
            return
        ...
        # ====== المنشورات ======
        if data == CB.POST_ADD:
            await CallbackHandlers._add_posts(query, context, user_id)
            return
        if data == CB.POST_PUB:
            await CallbackHandlers._publish_one(query, context, user_id)
            return
        if data == CB.POST_LIST:
            await CallbackHandlers._my_posts(query, context, user_id)
            return
        if data == CB.POST_REC:
            active = await ChannelRepository.get_active(user_id)
            if active:
        ...
        if data.startswith(CB.POST_CLEAR):
            ...
            return
        if data == CB.PUB_ALL:
            await CallbackHandlers._publish_all(query, context, user_id)
            return
        ...
        # ====== المجموعات ======
        if data == CB.GROUPS:
            await CallbackHandlers._my_groups(query, context, user_id)
            return
        if data.startswith(CB.GRP_SET):
            await CallbackHandlers._group_settings(query, context, user_id)
            return

        # ====== الإعدادات ======
        if data == CB.SETTINGS:
            await CallbackHandlers._settings(query, user_id)
            return
        ...
        if data.startswith("sec_"):
            await CallbackHandlers._security_toggle(query, context, user_id)
            return
        ...
        if data.startswith(CB.SEC_BANNED):
            ...
            await query.edit_message_text("🚫 **الكلمات المحظورة**", reply_markup=KeyboardFactory.banned_words(chat_id))
            return
        if data.startswith(CB.SEC_ENABLE_ALL):
            ...
            text = CallbackHandlers._build_security_text(settings)
            await query.edit_message_text(text, reply_markup=KeyboardFactory.security(chat_id), parse_mode="HTML")
            return
        ...
        if data.startswith(CB.SEC_DEL_PEN):
            chat_id = int(data.split(":")[-1])
            await query.edit_message_text("⚖️ اختر عقوبة حذف الوسائط:", reply_markup=KeyboardFactory.penalty(chat_id))
            return
        ...
        if data.startswith(CB.PENALTY):
            chat_id = int(data.split(":")[-1])
            await query.edit_message_text("⚖️ اختر العقوبة الأساسية:", reply_markup=KeyboardFactory.penalty(chat_id))
            return
        ...
        if data.startswith(CB.ADV_ACT):
            chat_id = int(data.split(":")[-1])
            await query.edit_message_text("🛠️ إجراءات متقدمة:", reply_markup=KeyboardFactory.advanced_actions(chat_id))
            return
        ...
        if data.startswith(CB.ACT_MUTE):
            chat_id = int(data.split(":")[-1])
            await query.edit_message_text("🔇 اختر المدة:", reply_markup=KeyboardFactory.mute_duration(chat_id))
            return
        ...
        # ====== الإحالات ======
        if data == CB.REFERRAL:
            await CallbackHandlers._referral(query, user_id)
            return
        if data == CB.REF_CLAIM:
            days = await ReferralRepository.claim(user_id)
            lang = await UserRepository.get_language(user_id)
            await query.edit_message_text(get_text(lang, 'referral_claimed', days=days) if days else get_text(lang,
                                                                                                            'no_referrals'))
            return
        if data == CB.REF_LIST:
        ...
        # ====== التذكيرات ======
        if data == CB.REMINDER:
            await CallbackHandlers._reminder(query, user_id)
            return
        ...
        if data == CB.REM_SET_LANG:
            kb = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("🔙", callback_data=CB.REMINDER)]
            ])
            await query.edit_message_text("🌐 اختر لغة الإشعارات:", reply_markup=kb)
            return
        if data.startswith(CB.REM_LANG):
            lang = data.split(":")[-1]
            await ReminderRepository.update_settings(user_id, notification_lang=lang)
            await CallbackHandlers._reminder(query, user_id)
            return

        # ====== الترجمة ======
        if data == CB.TRANSLATION:
            await CallbackHandlers._translation(query, user_id)
            return
        ...
        if data.startswith(CB.TRANS_SET):
            lang = data.split(":")[-1]
            await UserRepository.set_language(user_id, lang)
        ...
        if data.startswith(CB.AUTO_REPLY_MENU):
            ...
            await query.edit_message_text("📝 إدارة الردود التلقائية",
                                          reply_markup=KeyboardFactory.auto_reply_manage(chat_id))
            return
        if data.startswith(CB.AUTO_REPLY_TOGGLE):
            ...
            settings = await AutoReplyRepository.get_settings(chat_id)
            await query.edit_message_text("📝 إعدادات الردود التلقائية",
                                          reply_markup=KeyboardFactory.auto_reply_settings(chat_id, settings))
            return
        ...
        if data == CB.ADMIN:
            if user_id == CONFIG.PRIMARY_OWNER_ID or await BotAdminRepository.is_admin(user_id):
                await query.edit_message_text("👑 لوحة التحكم", reply_markup=KeyboardFactory.admin())
            else:
        ...
        if data.startswith("admin_"):
            await CallbackHandlers._admin_router(query, context, user_id)
            return
        ...
        if data.startswith("lang_"):
            lang = data.split("_")[-1]
            await UserRepository.set_language(user_id, lang)
            await query.answer(f"✅ تم تغيير اللغة إلى {lang}")
            await CommandHandlers.start(update, context)
            return
    ...
    # ====== دوال مساعدة خاصة ======
    @staticmethod
    async def _my_channels(query, user_id):
        channels = await ChannelRepository.get_all(user_id)
        if not channels:
    ...
    @staticmethod
    async def _add_posts(query, context, user_id):
        if not await UserRepository.has_active_subscription(user_id) and not await UserRepository.has_used_trial(
                user_id):
    ...
    @staticmethod
    async def _publish_one(query, context, user_id):
        if not await UserRepository.has_active_subscription(user_id) and not await UserRepository.has_used_trial(
                user_id):
            await query.edit_message_text("⚠️ اشتراك منتهٍ")
            return
    ...
    @staticmethod
    async def _my_posts(query, context, user_id):
        active = context.user_data.get('active_channel') or await ChannelRepository.get_active(user_id)
        if not active:
            await query.edit_message_text("⚠️ اختر قناة")
            return
    ...
    @staticmethod
    async def _publish_all(query, context, user_id):
        channels = await ChannelRepository.get_all(user_id)
        if not channels:
            return
        tasks = []
        ...
        await safe_send(context.bot, user_id, "✅ تم النشر للكل")
    ...
    @staticmethod
    async def _my_groups(query, context, user_id):
        ...
        if not valid:
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ أضف", url=f"https://t.me/{CONFIG.BOT_USERNAME}?startgroup")],
                                       [InlineKeyboardButton("🔙 رجوع", callback_data=CB.BACK)]])
    ...
    @staticmethod
    async def _group_settings(query, context, user_id):
    ...
    @staticmethod
    async def _settings(query, user_id):
        auto = "✅" if await UserRepository.get_auto_status(user_id) else "❌"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"⚙️ نشر تلقائي: {auto}", callback_data=CB.TOGGLE_AUTO)],
            [InlineKeyboardButton("🔙 رجوع", callback_data=CB.BACK)]
        ])
        await query.edit_message_text("⚙️ **الإعدادات**", reply_markup=kb)

    @staticmethod
    async def _security_toggle(query, context, user_id):
    ...
    @staticmethod
    async def _referral(query, user_id):
        stats = await ReferralRepository.get_stats(user_id)
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📋 قائمة الإحالات", callback_data=CB.REF_LIST)],
            [InlineKeyboardButton("🔙", callback_data=CB.BACK)]
        ])
        await query.edit_message_text(text, reply_markup=kb)

    @staticmethod
    async def _reminder(query, user_id):
        settings = await ReminderRepository.get_settings(user_id)
    ...
    @staticmethod
    async def _translation(query, user_id):
        lang = await UserRepository.get_language(user_id)
        kb = InlineKeyboardMarkup([
    ...
    @staticmethod
    async def _admin_router(query, context, user_id):
        data = query.data
        ...
        if data == CB.ADMIN_USERS:
            stats = await UserRepository.get_stats()
            await query.edit_message_text(f"👥 المستخدمين: {stats['users']}\n🚫 محظورين: {stats['banned']}")
            ...
            channels = await ChannelRepository.get_all(-1)  # -1 = جميع القنوات
            text = "📡 القنوات:\n" + "\n".join(
                [f"• `{ch['channel_id']}` - {ch['channel_name']} {'🚫' if ch['banned'] else '✅'}" for ch in
                 channels[:50]]) if channels else "لا توجد"
            await query.edit_message_text(text)
            ...
            text = "🚫 القنوات المحظورة:\n" + "\n".join(
                [f"• `{ch[0]}` - {ch[1]}" for ch in channels[:50]]) if channels else "لا توجد"
            await query.edit_message_text(text)
            ...
            text = "👥 المجموعات:\n" + "\n".join(
                [f"• `{r[0]}` - {r[1]} {'🚫' if r[3] else '✅'}" for r in rows]) if rows else "لا توجد"
            await query.edit_message_text(text)
            ...
            text = "🚫 المجموعات المحظورة:\n" + "\n".join(
                [f"• `{r[0]}` - {r[1]}" for r in rows[:50]]) if rows else "لا توجد"
            await query.edit_message_text(text)
            ...
            await query.edit_message_text(
                f"👥 {stats['users']} | 🚫 {stats['banned']} | 📝 {stats['posts']} | 👥 {stats['groups']} | 📡 {stats['channels']}")
            ...
            try:
                ...
                await safe_send(context.bot, user_id, f"✅ تم النسخ: {backup_file.name}")
            except Exception as e:
                await safe_send(context.bot, user_id, f"❌ فشل: {str(e)[:100]}")
            ...
            text = "📋 التذاكر:\n"
            for t in tickets:
                text += f"#{t['ticket_number']} - من `{t['user_id']}` ({t['username'] or 'لا يوجد'})\n{t['message'][:50]}...\n"
            await query.edit_message_text(text)
            ...
            text = "💬 الردود العامة:\n" + "\n".join(
                [f"• `{kw}`: {cnt} مرة" for kw, cnt in stats]) if stats else "لا توجد"
            await query.edit_message_text(text)
            ...
            text = "📋 الردود:\n" + "\n".join(
                [f"• `{r[0]}` → {r[1][:30]}... ({r[2]})" for r in replies]) if replies else "لا توجد"
            await query.edit_message_text(text)
...
class MessageHandlers:
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if state == UserState.WAIT_CHANNEL:
            ...
            try:
                ...
                if bot_member.status not in ['administrator', 'creator'] or not bot_member.can_post_messages:
                    await safe_send(context.bot, user_id, "❌ البوت ليس مشرفاً أو لا يملك صلاحية النشر")
                    StateManager.clear(user_id)
                    return
                result = await ChannelRepository.add(user_id, str(chat.id), chat.title or "بدون اسم")
                await safe_send(context.bot, user_id, "✅ تمت الإضافة" if result else "⚠️ موجودة")
            except Exception as e:
                await safe_send(context.bot, user_id, f"❌ {str(e)[:100]}")
            StateManager.clear(user_id)
            return
        ...
        if state == UserState.ADDING_POSTS:
            ...
            await safe_send(context.bot, user_id, f"✅ {len(session)}/{target} | متبقي {remaining}")
                ...
                try:
                    ...
                    if 0 <= h <= 23 and 0 <= m <= 59:
                        ...
                        if ch:
                            ...
                            await safe_send(context.bot, user_id, f"✅ تم تعيين وقت النشر {text}")
                    else:
        ...
        if state == UserState.WAIT_GROUP_BAN:
            ...
            if chat_id_ban and await is_authorized_in_group(context.bot, chat_id_ban, user_id):
                if len(text) >= 2:
                    added, exists = await SecurityRepository.add_banned_word(text.lower(), chat_id_ban, user_id)
                    if exists:
                        await safe_send(context.bot, user_id, f"⚠️ الكلمة '{text}' موجودة مسبقاً")
                    else:
                        await safe_send(context.bot, user_id, f"✅ تمت إضافة '{text}'")
                else:
        ...
        if state == UserState.WAIT_REM_GROUP_BAN:
            ...
            if chat_id_ban and await is_authorized_in_group(context.bot, chat_id_ban, user_id):
                if text:
                    await SecurityRepository.remove_banned_word(text.lower(), chat_id_ban)
                    await safe_send(context.bot, user_id, f"✅ تم حذف '{text}'")
                else:
                    await safe_send(context.bot, user_id, "❌ أرسل كلمة صحيحة")
            else:
                await safe_send(context.bot, user_id, "🔒 غير مصرح")
            StateManager.clear(user_id)
            return
        ...
        if state == UserState.WAIT_GLOBAL_BAN:
            if len(text) >= 2:
                added, exists = await SecurityRepository.add_banned_word(text.lower(), -1, user_id)
                if exists:
                    await safe_send(context.bot, user_id, f"⚠️ الكلمة '{text}' موجودة مسبقاً")
                else:
                    await safe_send(context.bot, user_id, f"✅ تمت إضافة '{text}'")
        ...
        if state == UserState.WAIT_REM_GLOBAL_BAN:
            if text:
                await SecurityRepository.remove_banned_word(text.lower(), -1)
                await safe_send(context.bot, user_id, f"✅ تم حذف '{text}'")
            else:
                await safe_send(context.bot, user_id, "❌ أرسل كلمة صحيحة")
            StateManager.clear(user_id)
        ...
        if state == UserState.WAIT_ADMIN_ADD:
            try:
                ...
                await safe_send(context.bot, user_id, f"✅ تمت إضافة {target}")
            except:
        ...
        if state == UserState.WAIT_ADMIN_REM:
            try:
                ...
                await safe_send(context.bot, user_id, f"✅ تمت إزالة {target}")
            except:
                await safe_send(context.bot, user_id, "❌ خطأ")
            StateManager.clear(user_id)
            return
        ...
        if state == UserState.WAIT_BROADCAST:
            ...
            await safe_send(context.bot, user_id, f"📨 تأكيد:\n{text[:200]}", reply_markup=kb)
            return
        ...
        if state == UserState.WAIT_UPDATE_CH:
            ...
            await safe_send(context.bot, user_id, f"✅ تم تعيين @{text.replace('@', '')}")
            StateManager.clear(user_id)
            return
        ...
        if state == UserState.WAIT_REM_DAYS:
            try:
                ...
                if 1 <= val <= 30:
                    ...
                    await safe_send(context.bot, user_id, f"✅ تم تعيين {val} أيام")
                else:
        ...
        if state == UserState.WAIT_CONTEST_ANSWER:
            ...
            if cid:
                await ContestRepository.participate(user_id, cid, text if text != '/skip' else "")
        ...
        if state == UserState.WAIT_AUTO_KEY:
            ...
            if keyword:
                ...
                await safe_send(context.bot, user_id, "✏️ أرسل الرد:")
            else:
        ...
        if state == UserState.WAIT_AUTO_REPLY:
            ...
            if chat_id_auto is not None and keyword:
                ...
                await safe_send(context.bot, user_id, f"✅ تم إضافة رد لـ '{keyword}'")
            else:
            ...
            if chat_id_auto is not None:
                ...
                if keyword:
                    if await AutoReplyRepository.remove_reply(chat_id_auto, keyword):
                        await safe_send(context.bot, user_id, f"✅ تم حذف الرد لـ '{keyword}'")
                    else:
                        await safe_send(context.bot, user_id, f"❌ لا يوجد رد لـ '{keyword}'")
                else:
                    await safe_send(context.bot, user_id, "❌ كلمة غير صالحة")
            else:
                await safe_send(context.bot, user_id, "❌ خطأ في البيانات")
        ...
        # ====== إعدادات أخرى ======
        if state == UserState.WAIT_SENDCODE:
            try:
                await SettingRepository.set('allowed_sendcode_user', str(int(text)))
                await safe_send(context.bot, user_id, "✅ تم")
            except:
                await safe_send(context.bot, user_id, "❌ خطأ")
            StateManager.clear(user_id)
        ...
        if state == UserState.WAIT_MAX_LEN:
            try:
                ...
                if val >= 0:
                    ...
                    await safe_send(context.bot, user_id, f"✅ {val}")
            except:
                pass
            StateManager.clear(user_id)
            return
        ...
        if state == UserState.SUPPORT_MODE:
            if text:
                ...
                await TicketRepository.save(user_id, update.effective_user.username or "", text, ticket_num)
...
class MutePenalty(PenaltyStrategy):
    async def apply(self, bot, chat_id: int, user_id: int, **kwargs) -> Tuple[bool, str]:
        ...
        until = datetime.utcnow() + timedelta(minutes=duration) if duration else None
        try:
...
class WarnPenalty(PenaltyStrategy):
    async def apply(self, bot, chat_id: int, user_id: int, **kwargs) -> Tuple[bool, str]:
        async def _warn():
            ...
            await DB.execute("INSERT OR REPLACE INTO user_warnings (user_id, chat_id, warnings) VALUES (?,?,?)",
                             (user_id, chat_id, w))
            return w
...
async def apply_penalty(bot, chat_id: int, user_id: int, penalty: str, duration: int = 0, reason: str = "",
                        moderator: int = None) -> Tuple[bool, str]:
    if user_id == CONFIG.PRIMARY_OWNER_ID:
            ...
            try:
                for u in await ReminderRepository.get_users_needing_reminder():
                    try:
                        lang = u.get('lang', 'ar')
                        days = u['days_left']
                        text = f"⚠️ اشتراكك ينتهي خلال {days} أيام" if lang == 'ar' else f"⚠️ Your subscription expires in {days} days"
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if success:
        await safe_send(context.bot, user_id, "✅ تم الدفع بنجاح، تم تفعيل اشتراكك!")
    else:
...
# =====================================================================
async def main():
    logger.info(f"🚀 Starting {CONFIG.BOT_NAME} v5.0.0-final")
    ...
    # إعداد طلب HTTP
    if CONFIG.USE_PROXY:
        request = HTTPXRequest(proxy_url=CONFIG.PROXY_URL, read_timeout=60, write_timeout=30, connect_timeout=30,
                               connection_pool_size=CONFIG.MAX_CONNECTIONS)
    else:
        request = HTTPXRequest(read_timeout=60, write_timeout=30, connect_timeout=30,
                               connection_pool_size=CONFIG.MAX_CONNECTIONS)
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.GROUPS & ~filters.COMMAND, MessageHandlers.handle),
                    group=1)
    ...
    # === بدء التشغيل ===
    hostname = os.getenv("RENDER_EXTERNAL_HOSTNAME") or os.getenv("RAILWAY_PUBLIC_DOMAIN") or os.getenv(
        "HEROKU_APP_NAME")
    if hostname:
...
if __name__ == "__main__":
    print(f"🌿 {CONFIG.BOT_NAME} v5.0.0-final - @RelaxMgr")
