...
"""
ريلاكس مانيجر - النسخة النهائية المعدلة بالكامل (مع تصحيح الأخطاء وإضافة الترجمات والردود المنفصلة)
الإصدار: 23.0.1-fixed-full
المطور: @RelaxMgr
...
# ===================================================================
# 1. تثبيت الحزم تلقائياً (نفس الكود الأصلي)
# ===================================================================
...
AUTO_REPLIES_FILE = BASE_PATH / "auto_replies.json"  # [FIX] ملف الردود التلقائية المنفصل

# ===================================================================
# 5. تحميل الترجمات (استيراد من locales.py)
# ===================================================================
try:
    from locales import LOCALES, get_text
except ImportError:
    # تعريف الترجمات في حال عدم وجود الملف (للتشغيل المستقل)
    LOCALES = {
        'ar': {
            ...
            'referral_claimed': "✅ {days} يوم" if days else "❌ لا يوجد",
        ...
        'en': {
            ...
            'referral_claimed': "✅ {days} days" if days else "❌ Nothing",
...
# [FIX] تحميل الردود التلقائية من ملف JSON منفصل
...
# دالة لاستيراد الردود التلقائية إلى قاعدة البيانات
        ...
        try:
            ...
            for key, timestamps in _flood_cache.items():
                if not timestamps:
                    ...
                    # حذف الطوابع الأقدم من 60 ثانية
...
async def init_db():
    async def _init(conn):
        ...
        # الردود التلقائية مع الإحصائيات
        await conn.execute("""
...
async def is_authorized_in_group(bot, chat_id: int, user_id: int) -> bool:
    ...
    try:
        ...
        authorized = await db_is_hidden_owner(chat_id, user_id) or await execute_db(lambda c: c.execute("SELECT 1 FROM hidden_admins WHERE chat_id=? AND admin_id=?", (chat_id, user_id)) or c.fetchone()) is not None
    if CACHETOOLS_AVAILABLE:
...
# ===================================================================
# 19. الردود التلقائية (مع دالة الحذف)
# ===================================================================
...
class CallbackData:
    ...
    ADMIN_DEL_REPLY = "admin:del_reply"  # [FIX] تم تغيير هذا لاستخدام حالة جديدة
    ...
    # [FIX] إضافة أزرار جديدة للردود التلقائية
...
class UserState(Enum):
    ...
    # [FIX] حالات جديدة للردود التلقائية
...
# [FIX] إضافة كيبورد لإدارة الردود التلقائية (مع أزرار إضافة/حذف/عرض)
...
# ===================================================================
# 24. دوال الكولباك الأساسية (المعالج الشامل المعدل)
# ===================================================================
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """المعالج الشامل لجميع أزرار الكولباك."""
    ...
    # [FIX] إدارة الردود التلقائية (إضافة، حذف، عرض)
    ...
    # ===== الردود التلقائية - القائمة (توافق مع الكود القديم) =====
    if data.startswith(CallbackData.AUTO_REPLY_MENU_PREFIX) and not data.startswith(CallbackData.AUTO_REPLY_ADD) and not data.startswith(CallbackData.AUTO_REPLY_DEL) and not data.startswith(CallbackData.AUTO_REPLY_LIST):
        chat_id = int(data.split(":")[-1])
        await query.edit_message_text("📝 إدارة الردود التلقائية", reply_markup=get_auto_reply_management_keyboard(chat_id))
        return
...
async def admin_router_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == CallbackData.ADMIN_USERS:
        ...
        await query.edit_message_text(text)
    # [FIX] ADMIN_DEL_REPLY - استخدام حالة جديدة
    elif data == CallbackData.ADMIN_DEL_REPLY:
        context.user_data['state'] = UserState.WAITING_AUTO_REPLY_DELETE
        context.user_data['auto_reply_chat_id'] = -1  # -1 يعني الردود العامة
        await query.edit_message_text("✏️ أرسل الكلمة المفتاحية لحذف الرد:")
...
async def syncgroup_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    تفعيل المجموعة - للمشرفين فقط (مع دعم المشرفين المخفيين).
    - يتحقق من صلاحيات البوت أولاً.
    - يتحقق من أن المستخدم مشرف حقيقي (أو مالك).
    - يتعامل مع المشرفين المخفيين (Anonymous Admins) باكتشاف المعرف الحقيقي.
    - يسجل المجموعة في حساب المشرف، فتظهر في قائمة 'مجموعاتي'.
    - يرفض العضو العادي برسالة واضحة.
    """
    chat = update.effective_chat
    if chat.type not in ['group', 'supergroup']:
    ...
    # 1. التحقق من صلاحيات البوت (يجب أن يكون مشرفاً)
    try:
    ...
    # 2. التحقق من صلاحيات المستخدم (هل هو مشرف حقيقي؟)
    try:
    ...
    # 3. التعامل مع المشرفين المخفيين (Anonymous Admins)
    real_user_id = user_id
    if user_id == ANONYMOUS_ADMIN_ID:  # 1087968824
        try:
            admins = await context.bot.get_chat_administrators(chat_id)
            # نبحث عن المالك الحقيقي
            for a in admins:
                if a.status == 'creator':
                    real_user_id = a.user.id
                    break
            ...
            if real_user_id == user_id:
                for a in admins:
                    if a.user.id != ANONYMOUS_ADMIN_ID:
                        real_user_id = a.user.id
                        break
            is_admin = True  # المشرف المخفي يعتبر مشرفاً
        except:
            pass

    # 4. إذا كان المستخدم ليس مشرفاً (ولا حتى مخفياً) → نرفضه
    if not is_admin:
        await safe_send_markdown(context.bot, user_id, "🔒 **غير مصرح لك.**\nهذا الأمر متاح فقط للمشرفين والمالكين.")
        return

    # 5. تسجيل المجموعة في قاعدة البيانات (باستخدام المعرف الحقيقي)
    chat_name = chat.title or "بدون اسم"
    chat_username = chat.username or None
    await db_register_group(chat_id, chat_name, real_user_id, chat_username)

    # 6. إضافة المشرف الحقيقي إلى جدول user_groups_link (حتى تظهر المجموعة في 'مجموعاتي')
    await execute_db(lambda c: c.execute(
        "INSERT OR IGNORE INTO user_groups_link (user_id, chat_id) VALUES (?,?)",
        (real_user_id, chat_id)
    ) or c.commit())

    # 7. تسجيل المالك المخفي (للمشرف) ومزامنة المشرفين
    await db_register_hidden_owner_group(chat_id, real_user_id)
    invalidate_auth_cache(chat_id, real_user_id)
    count = await db_sync_group_admins(chat_id, context.bot, real_user_id)

    # 8. إلغاء حظر المجموعة إذا كانت محظورة
    if await db_is_group_banned(chat_id):
        await db_set_group_ban(chat_id, False)

    # 9. تسجيل الإجراء في سجل الأدمن
    await db_log_admin_action(chat_id, real_user_id, "syncgroup", target_id=0, reason="تفعيل البوت (مشرف)")

    # 10. إرسال تقرير تأكيد للمشرف
    report = f"""
✅ **تم تفعيل المجموعة بنجاح!**

📌 **المجموعة:** {chat_name}
🆔 المعرف: `{chat_id}`
👑 **المالك المخفي:** `{real_user_id}`
👥 **عدد المشرفين المسجلين:** {count}

🔹 **الآن ستظهر هذه المجموعة في قائمة '👥 مجموعاتي'** داخل البوت.
...
🔹 استخدم `/panel` للتحكم السريع في المجموعة.
    ...
    # 11. إرسال رسالة ترحيب في المجموعة (اختياري)
    try:
        await safe_send_markdown(context.bot, chat_id, f"✅ تم تفعيل البوت بواسطة المشرف `{real_user_id}`.")
    except:
        pass
...
async def panel_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔒 قفل", callback_data=f"{CallbackData.PANEL_LOCK_PREFIX}{chat_id}"),
         InlineKeyboardButton("🔓 فتح", callback_data=f"{CallbackData.PANEL_UNLOCK_PREFIX}{chat_id}")],
        [InlineKeyboardButton("🔙 إغلاق", callback_data=CallbackData.PANEL_CLOSE)]
    ])
    await safe_send_markdown(context.bot, user_id, f"📋 لوحة تحكم المجموعة\nالحالة: {'مقفلة' if is_locked else 'مفتوحة'}", reply_markup=kb)
...
# ===================================================================
# 32. دوال قاعدة البيانات الإضافية (جديدة)
# ===================================================================
...
async def db_get_metrics():
    async def _g(conn):
        ...
        # [FIX] تحسين الاستعلام لحساب المستخدمين النشطين بدقة
...
async def main():
    ...
    # [FIX] استيراد الردود التلقائية من ملف JSON
...
if __name__ == "__main__":
    print("🌿 ريلاكس مانيجر v23.0.1-fixed-full (مع جميع التعديلات)")
