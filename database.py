...
- ✅ إصلاح دالة get_user_groups لتشمل المجموعات المضافة بواسطة added_by
"""
...
class TimeUtils:
    ...
    def safe_parse_iso(date_str: Optional[str]) -> Optional[datetime]:
        ...
        if not date_str:
            return None
        try:
            # محاولة parse بالتنسيق القياسي
            return datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S')
        except ValueError:
            try:
                # محاولة parse بصيغة ISO
...
class Database:
    ...
    async def get_all_users(self) -> List[Dict]:
        ...
        return await self.fetchall(
            "SELECT user_id, banned FROM users ORDER BY user_id"
        )
            ...
            async with self._lock:
                async with self._get_connection() as conn:
                    ...
                    await conn.execute(
                        ...
                        (
                            ch_db_id,
                            (TimeUtils.utc_now() + timedelta(minutes=12)).strftime('%Y-%m-%d %H:%M:%S')
                        )
                    )
            ...
            async with self._lock:
                async with self._get_connection() as conn:
                    ...
                    for i in range(0, len(posts), 100):
                        batch = posts[i:i+100]
                        vals = [
                            (channel_db_id, (t or "")[:4096], m, f, TimeUtils.sql_iso())
                            for t, m, f in batch
                        ]
    ...
    async def get_next_post(self, channel_db_id: int) -> Optional[Dict]:
        """جلب المنشور التالي مع إعادة التدوير"""
        ...
        if post:
            ...
            if post.get('fail_count', 0) > 0:
                await self.execute(
                    "UPDATE posts SET fail_count = 0 WHERE id = ?",
                    (post['id'],)
                )
            return post
    ...
    async def get_user_groups(self, user_id: int) -> List[Dict]:
        """جلب مجموعات المستخدم (تشمل added_by)"""
        return await self.fetchall(
    ...
    async def is_anonymous_admin(self, chat_id: int, user_id: int) -> bool:
        """التحقق من كون المستخدم مشرف مجهول"""
        result = await self.fetchval(
    ...
    async def add_banned_word(self, word: str, chat_id: int, added_by: int) -> Tuple[bool, bool]:
        """إضافة كلمة محظورة
        Returns: (success, already_exists)
        """
        try:
            ...
            # التحقق من الحد الأقصى للكلمات العامة
            if chat_id == -1:
    ...
    async def update_next_publish(self, channel_db_id: int) -> bool:
        ...
        last_publish = await self.fetchval(
        ...
        try:
            async with self._lock:
                async with self._get_connection() as conn:
                    ...
                    # إضافة الاشتراك
                    current_end = await self.fetchval(
            ...
            async with self._lock:
                async with self._get_connection() as conn:
                    ...
                    await conn.execute(
                        ...
                        (user_id, TimeUtils.sql_iso(), gift_code['id'])
                    )
    ...
    async def add_penalty(self, user_id: int, chat_id: int, penalty_type: str,
        ...
        try:
            ...
            async with self._get_connection() as conn:
                # إزالة العقوبات النشطة المماثلة
                await conn.execute(
