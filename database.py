...
class Database:
    ...
    async def _create_tables(self, conn) -> None:
        ...
        await conn.execute("INSERT OR IGNORE INTO settings VALUES ('last_backup', '')")
...
async def add_hidden_owner(self, chat_id: int, owner_id: int) -> bool:
    try:
        await self.execute(
        ...
        logger.error(f"❌ Error in add_hidden_owner: {e}", exc_info=True)
        return False

async def remove_hidden_owner(self, chat_id: int, owner_id: int) -> bool:
    try:
        await self.execute("DELETE FROM hidden_owner_groups WHERE chat_id=? AND owner_id=?", (chat_id, owner_id))
        return True
    except Exception as e:
        logger.error(f"❌ Error in remove_hidden_owner: {e}", exc_info=True)
        return False
...
async def get_hidden_owners(self, chat_id: int) -> List[int]:
    try:
        ...
        logger.error(f"❌ Error in get_hidden_owners: {e}", exc_info=True)
        return []
...
async def is_hidden_owner(self, chat_id: int, user_id: int) -> bool:
    try:
        ...
        logger.error(f"❌ Error in is_hidden_owner: {e}", exc_info=True)
        return False
...
async def is_hidden_admin(self, chat_id: int, user_id: int) -> bool:
    try:
        ...
        logger.error(f"❌ Error in is_hidden_admin: {e}", exc_info=True)
        return False
...
async def get_admin_logs(self, chat_id: int, limit: int = 20) -> List[Dict]:
    ...
    async def get_auto_reply_settings(self, chat_id: int) -> Dict:
        """جلب إعدادات الردود التلقائية مع إنشاء افتراضي إذا لم توجد"""
        try:
            ...
            return {'enabled': 0, 'only_admins': 0, 'ignore_bots': 1, 'updated_at': TimeUtils.utc_iso()}
        except Exception as e:
    ...
    async def add_auto_reply(self, chat_id: int, keyword: str, reply: str,
        ...
        try:
            ...
            # محاولة الإدراج مع معالجة التعارض
            await self.execute(
                """INSERT INTO auto_replies 
                   ...
                   reply=excluded.reply, reply_type=excluded.reply_type,
                   reply_media_id=excluded.reply_media_id, reply_buttons=excluded.reply_buttons,
                   created_at=excluded.created_at, is_active=1""",
                (chat_id, keyword, reply, reply_type, media_id, buttons, TimeUtils.utc_iso())
            )
            _auto_reply_cache.invalidate(chat_id)
            return True
        except Exception as e:
    ...
    async def get_auto_reply(self, keyword: str, chat_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if row:
                ...
                _auto_reply_cache.set(cache_key, result)
                return result

            # البحث في الردود العامة (chat_id = -1)
            row = await self.fetchone(
    ...
    async def get_schedule(self, channel_id: int) -> Dict:
        ...
        try:
            ...
            return await self.get_schedule(channel_id)
        except Exception as e:
    ...
    async def get_referral_stats(self, user_id: int) -> Dict:
        ...
        try:
            ...
            claimed = (await self.fetchval("SELECT COALESCE(claimed_reward_days,0) FROM referral_rewards WHERE user_id=?", (user_id,))) or 0
            available = (await self.fetchval("SELECT COALESCE(total_reward_days,0) - COALESCE(claimed_reward_days,0) FROM referral_rewards WHERE user_id=?", (user_id,))) or 0
    ...
    async def get_reminder_settings(self, user_id: int) -> Dict:
        ...
        try:
            ...
            return {'subscription_reminder': 1, 'daily_stats_reminder': 0, 'weekly_report': 1, 'reminder_days_before': 3, 'notification_lang': 'ar'}
        except Exception as e:
    ...
    async def get_users_for_reminder(self) -> List[Dict]:
        """جلب المستخدمين المستحقين للتذكير"""
        try:
            rows = await self.fetchall("""
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        await DB.add_auto_reply(-1, "سلام", "وعليكم السلام")
        reply = await DB.get_auto_reply("سلام", -1)
        print(f"✅ الرد التلقائي: {reply}")
