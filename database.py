...
"""
database.py - قاعدة البيانات المتكاملة (كاملة 100%)
"""
...
class Database:
    ...
    async def _get_connection(self):
        async with aiosqlite.connect(str(PATHS.DB), timeout=30) as conn:
    ...
    async def _init_default_data(self, conn) -> None:
        ...
        for name, desc, price, days, max_ch, max_p in plans:
            await conn.execute(
        ...
        try:
            ...
            for word in BANNED_WORDS:
                await conn.execute(
                    ...
                    (word.lower(), -1, CONFIG.PRIMARY_OWNER_ID, TimeUtils.utc_iso())
                )
        except ImportError:
            pass
        await conn.commit()
    ...
    async def register_user(self, user_id, username="", first_name=""):
        ...
        if row:
            await self.execute("UPDATE users SET username=?, first_name=?, updated_at=? WHERE user_id=?", (username, first_name, TimeUtils.utc_iso(), user_id))
            return True
        code = secrets.token_urlsafe(6)
        await self.execute("INSERT INTO users (user_id, username, first_name, referral_code, trial_used, created_at, updated_at) VALUES (?,?,?,?,0,?,?)", (user_id, username, first_name, code, TimeUtils.utc_iso(), TimeUtils.utc_iso()))
        return True
    ...
    async def activate_trial(self, user_id):
        end = (TimeUtils.utc_now() + timedelta(days=30)).isoformat()
        await self.execute("UPDATE users SET trial_used=1, subscription_end=? WHERE user_id=?", (end, user_id))
        return 30
    ...
    async def add_referral(self, referrer_id, referred_id):
        ...
        try:
            ...
            await self.execute("INSERT INTO referral_rewards (user_id, referral_count, total_reward_days, last_referral_date) VALUES (?,1,3,?) ON CONFLICT(user_id) DO UPDATE SET referral_count=referral_count+1, total_reward_days=total_reward_days+3, last_referral_date=?", (referrer_id, TimeUtils.utc_iso(), TimeUtils.utc_iso()))
            return True
    ...
    async def claim_referral_reward(self, user_id):
        ...
        if row and row[0]:
            current = TimeUtils.safe_parse_iso(row[0])
            new_end = (current + timedelta(days=av)).isoformat() if current else (TimeUtils.utc_now() + timedelta(days=av)).isoformat()
        else:
            new_end = (TimeUtils.utc_now() + timedelta(days=av)).isoformat()
        await self.execute("UPDATE users SET subscription_end=? WHERE user_id=?", (new_end, user_id))
        return av

    async def get_referrals_list(self, user_id):
        rows = await self.fetchall("SELECT referred_id FROM referrals WHERE referrer_id=?", (user_id,))
        return [r[0] for r in rows]

    # ========== القنوات ==========
    async def add_channel(self, user_id, channel_id, channel_name):
        row = await self.fetchone("SELECT id FROM user_channels WHERE user_id=? AND channel_id=?", (user_id, channel_id))
        if row:
            return row[0]
        async with self._get_connection() as conn:
            cur = await conn.execute("INSERT INTO user_channels (user_id, channel_id, channel_name, created_at) VALUES (?,?,?,?)", (user_id, channel_id, channel_name, TimeUtils.utc_iso()))
            ch_db_id = cur.lastrowid
            await conn.execute("INSERT OR IGNORE INTO schedule (channel_db_id, next_publish_date) VALUES (?,?)", (ch_db_id, (TimeUtils.utc_now() + timedelta(seconds=720)).isoformat()))
            await conn.commit()
            return ch_db_id

    async def get_user_channels(self, user_id):
        rows = await self.fetchall("SELECT id, channel_id, channel_name, banned FROM user_channels WHERE user_id=?", (user_id,))
        return [dict(r) for r in rows]

    async def get_active_channel(self, user_id):
        row = await self.fetchone("SELECT active_channel FROM users WHERE user_id=?", (user_id,))
        if row and row[0]:
            return row[0]
        row2 = await self.fetchone("SELECT id FROM user_channels WHERE user_id=? AND banned=0 LIMIT 1", (user_id,))
        return row2[0] if row2 else None

    async def set_active_channel(self, user_id, channel_id):
    ...
    async def delete_channel(self, user_id, channel_id):
        await self.execute("DELETE FROM user_channels WHERE id=? AND user_id=?", (channel_id, user_id))

    # ========== المنشورات ==========
    async def add_posts(self, channel_id, posts):
    ...
    async def get_unpublished_posts_count(self, channel_id):
        return await self.fetchval("SELECT COUNT(*) FROM posts WHERE channel_db_id=? AND published=0", (channel_id,)) or 0

    async def get_user_unpublished_count(self, user_id):
        return await self.fetchval("SELECT COUNT(*) FROM posts p JOIN user_channels uc ON p.channel_db_id=uc.id WHERE uc.user_id=? AND p.published=0", (user_id,)) or 0

    async def get_user_total_posts(self, user_id):
        return await self.fetchval("SELECT COUNT(*) FROM posts p JOIN user_channels uc ON p.channel_db_id=uc.id WHERE uc.user_id=?", (user_id,)) or 0

    async def get_next_post(self, channel_id):
        row = await self.fetchone("SELECT id, text, media_type, media_file_id FROM posts WHERE channel_db_id=? AND published=0 ORDER BY created_at LIMIT 1", (channel_id,))
        return dict(row) if row else None

    async def get_user_posts(self, channel_id, limit=10):
    ...
    async def increment_post_fail(self, post_id):
        await self.execute("UPDATE posts SET fail_count=fail_count+1 WHERE id=?", (post_id,))

    async def delete_post(self, post_id, user_id, channel_id):
    ...
    async def reset_posts(self, channel_id):
    ...
    # ========== المجموعات ==========
    async def register_group(self, chat_id, chat_name, user_id, username=None):
    ...
    async def sync_group_admins(self, chat_id, admin_ids):
    ...
    # ========== الأمان ==========
    async def get_security_settings(self, chat_id):
    ...
    async def add_banned_word(self, word, chat_id, added_by):
        try:
    ...
    async def remove_banned_word(self, word, chat_id):
    ...
    async def get_auto_reply_settings(self, chat_id):
        ...
        return {'enabled': 0, 'only_admins': 0}

    async def update_auto_reply_settings(self, chat_id, **kwargs):
    ...
    async def add_auto_reply(self, chat_id, keyword, reply):
    ...
    async def update_reminder_settings(self, user_id, **kwargs):
        updates = [f"{k}=?" for k in kwargs]
        vals = list(kwargs.values()) + [user_id]
        await self.execute(f"UPDATE user_reminder_settings SET {', '.join(updates)} WHERE user_id=?", vals)

    # ========== المسابقات ==========
    async def create_contest(self, creator_id, title, description, prize, end_date):
        async with self._get_connection() as conn:
    ...
    async def declare_winner(self, contest_id, winner_id):
    ...
    async def get_contest_winners(self, limit=10):
    ...
    async def get_force_subscribe_channel(self):
        return await self.get_setting('force_subscribe_channel')

    async def get_publish_interval(self):
    ...
    async def get_user_invoices(self, user_id, limit=10):
